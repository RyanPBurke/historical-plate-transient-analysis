...
import tempfile
import time
from v095a_runtime_rc5 import ProjectLock, BoundedHTTP

CONTRACT_SHA = "0ed31dcd20e5b75c46fb6ce5405ee0fbcd712b3201175a5b9316f67d0aa344d2"
...
CONTRACT = HERE / "v095a_stageA_contract_rc5.json"
SQL_FILE = HERE / "v095a_stageA_queries_rc5.sql"
BINS_FILE = HERE / "v095a_stageA_bins_rc5.json"
...
RELEASE_REL = Path("pipeline_v0.2.0/research/prospective_freezes/v095a_stageA_aggregate_inventory_rc5")
...
def verify_release_manifest():
    ...
    for line in RELEASE_MANIFEST.read_text(encoding="utf-8-sig").splitlines():
        ...
        entries[name] = digest
    required = {
        "v095a_source_matcher_design_rc5.md", "v095a_stageA_contract_rc5.json",
        "v095a_stageA_queries_rc5.sql", "v095a_stageA_bins_rc5.json",
        "run_v095a_stageA_inventory_rc5.py", "create_v095a_stageA_rc5_freeze_evidence.ps1",
        "run_v095a_stageA_rc5_selftest.ps1", "run_v095a_stageA_rc5_preflight.ps1",
        "run_v095a_stageA_rc5_production.ps1", ".gitattributes",
        "v095a_runtime_rc5.py", "v095a_jobs_rc5.py", "test_v095a_rc5.py",
        "test_sql_rc5.mjs", "package.json", "package-lock.json",
    }
...
def self_test():
    ...
    import unittest
    import test_v095a_rc5
    suite = unittest.defaultTestLoader.loadTestsFromModule(test_v095a_rc5)
    result = unittest.TextTestRunner(verbosity=2).run(suite)
    if not result.wasSuccessful():
        raise SystemExit("SELFTEST_HOLD: regression failure")
    print("v095a Stage A RC5 SELF-TEST PASS")
    print("Python lifecycle/parser/lock regressions passed; PostgreSQL SQL suite is test_sql_rc5.mjs")
...
def preflight(project: Path, repo: Path, write_plan=True, owner=None):
    ...
    if contract.get("contract_id") != "v095a_stageA_aggregate_inventory_rc5":
        raise SystemExit("RELEASE_HOLD: contract id")
    proof = project / "research/proof_archives/v094y_rc5_proof_archive.zip"
    ...
    if write_plan:
        work = project / "work/v095a_stageA_aggregate_inventory_rc5"
        plan_dir = work / "plan"
...
def run_batch(session, family, batch_no, query_bytes, expected_fields, validator, expected_arg,
    ...
    from v095a_jobs_rc5 import run_batch as execute_batch
...
def _run_production_locked(project, repo, freeze_commit, freeze_evidence, token_env, owner):
    owner.require(project)
    if (project / "results/applause_dr4_v095a_stageA_aggregate_inventory_rc5").exists():
    ...
    staging = Path(tempfile.mkdtemp(prefix='v095a_rc5_staging_', dir=results))
    final = project / "results/applause_dr4_v095a_stageA_aggregate_inventory_rc5"
    ...
    summary = {
        "analysis_kind": "v095a_stageA_aggregate_inventory_rc5",
        "status": "COMPLETE",
    ...
    print("v095a Stage A RC5 PRODUCTION PASS")
...
def main():
    ...
    if a.preflight_only:
        ...
        print("v095a Stage A RC5 PREFLIGHT PASS")
