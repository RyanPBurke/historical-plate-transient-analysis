#!/usr/bin/env python3
"""Order 11 / raw match 3 — target-independent local Gaia astrometry v044.
...
OUT = ROOT / "results" / "order11_followup_match3_v044"
CACHE = ROOT / "results" / "order11_followup_match3_v043a" / "gaia_local_reference_cache"
...
EXPECTED_NATIVE_POLICY_SHA = "44fc3453c3291a7cbe72894d781729a30943ad540aa169b2c0897b446c5c8ec7"
...
def main():
    print("="*120)
    print("ORDER 11 — MATCH 3 INDEPENDENT-GAIA SPARSE-FIELD ASTROMETRY v044")
    print("="*120)
    ...
    required = [PAIR_REPORT, RAW_MATCHES, POSS_ALL, DASCH_ALL, CONTROL_REPORT,
                TIMING_CENSUS, V042_REPORT, V042_SOURCES, DETECTOR, METHOD, NATIVE_POLICY, ADJ_POLICY]
    for p in required:
        if not p.is_file():
    ...
    hash_guards = {
        ...
        "native_policy": sha_file(NATIVE_POLICY) == EXPECTED_NATIVE_POLICY_SHA,
        "candidate_adjudication_policy": sha_file(ADJ_POLICY) == EXPECTED_ADJ_POLICY_SHA,
    ...
    # General sparse-field fallback learned from v043a: if the common-same-Gaia pool is
    # underpopulated, solve each archive independently against propagated Gaia. This is
    # predeclared for all future sparse fields, not tuned to match 3. It remains diagnostic.
    if selected is None:
        def archive_offsets(ms):
            out=[]
            for r in ms:
                if r["sep_target_arcmin"] > 30.0:
                    continue
                g=SkyCoord(r["gaia_ra_1953_deg"]*u.deg,r["gaia_dec_1953_deg"]*u.deg)
                c=SkyCoord(r["ra_deg"]*u.deg,r["dec_deg"]*u.deg)
                e,n=g.spherical_offsets_to(c)
                q=dict(r)
                q["archive_minus_gaia_east_arcsec"]=float(e.arcsec)
                q["archive_minus_gaia_north_arcsec"]=float(n.arcsec)
                out.append(q)
            return out

        p_ind=archive_offsets(pm)
        d_ind=archive_offsets(dm)
        if len(p_ind) < SPARSE_MIN_REFERENCES or len(d_ind) < SPARSE_MIN_REFERENCES:
            result = {
                "status": "COMPLETE",
                "analysis_kind": "order11_match3_independent_gaia_sparse_astrometry_v044",
                "classification": "INSUFFICIENT_INDEPENDENT_GAIA_REFERENCES",
                "common_reference_counts": {str(k): v for k,v in counts.items()},
                "independent_reference_counts": {"POSS": len(p_ind), "DASCH": len(d_ind)},
                "minimum_sparse_references_per_archive": SPARSE_MIN_REFERENCES,
                "sparse_fallback_policy": "translation-only median against independently matched propagated Gaia; inner 30 arcsec and target Gaia source excluded; diagnostic confidence only",
                "detector_rerun": False, "science_image_pixels_read": False, "candidate_state_mutation": False,
                "interpretation_boundary": "Insufficient independent references is not evidence for or against transience.",
            }
            write_json(OUT / "order11_match3_sparse_astrometry_report_v044.json", result)
            print("\nSPARSE LOCAL ASTROMETRY: INSUFFICIENT REFERENCES")
            print(f"Independent refs: POSS={len(p_ind)} DASCH={len(d_ind)}")
            return

        med_pe=float(np.median([r["archive_minus_gaia_east_arcsec"] for r in p_ind]))
        med_pn=float(np.median([r["archive_minus_gaia_north_arcsec"] for r in p_ind]))
        med_de=float(np.median([r["archive_minus_gaia_east_arcsec"] for r in d_ind]))
        med_dn=float(np.median([r["archive_minus_gaia_north_arcsec"] for r in d_ind]))
        ...
        p_loo=loo_radius(p_ind,"archive_minus_gaia_east_arcsec","archive_minus_gaia_north_arcsec")
        d_loo=loo_radius(d_ind,"archive_minus_gaia_east_arcsec","archive_minus_gaia_north_arcsec")

        if corrected_pd <= STRICT_ARCSEC and p_gaia_corr <= STRICT_ARCSEC and d_gaia_corr <= STRICT_ARCSEC:
            classification="SPARSE_DIAGNOSTIC_SUPPORTS_PERSISTENT_GAIA_SOURCE_ASSOCIATION"
        elif corrected_pd <= STRICT_ARCSEC and p_gaia_corr <= DIAGNOSTIC_ARCSEC and d_gaia_corr <= DIAGNOSTIC_ARCSEC:
            classification="SPARSE_DIAGNOSTIC_SUPPORTS_GAIA_ASSOCIATION_WITHIN_5ARCSEC"
        elif corrected_pd <= STRICT_ARCSEC:
            classification="SPARSE_DIAGNOSTIC_SUPPORTS_CROSS_OBSERVATORY_MATCH_NOT_TARGET_GAIA"
        else:
            classification="SPARSE_DIAGNOSTIC_DOES_NOT_SUPPORT_STRICT_RAW_COINCIDENCE"

        write_csv(OUT / "order11_match3_poss_independent_gaia_refs_v044.csv", p_ind, sorted({k for r in p_ind for k in r if k != "coord"}))
        write_csv(OUT / "order11_match3_dasch_independent_gaia_refs_v044.csv", d_ind, sorted({k for r in d_ind for k in r if k != "coord"}))
        write_json(OUT / "order11_match3_sparse_astrometry_report_v044.json", {
            "status":"COMPLETE",
            "analysis_kind":"order11_match3_independent_gaia_sparse_astrometry_v044",
            "classification":classification,
            "physical_overlap":{"start_utc":start.isoformat(),"end_utc":end.isoformat(),"actual_overlap_s":2700.0,"registration_epoch_utc":target_time.utc.isot},
            "candidate_adjudication_policy":{"path":str(ADJ_POLICY.relative_to(ROOT)),"sha256":sha_file(ADJ_POLICY)},
            "fixed_policy":{"primary_common_min_refs":MIN_COMMON_REFERENCES,"sparse_min_refs_per_archive":SPARSE_MIN_REFERENCES,"sparse_max_window_arcmin":30.0,"science_exclusion_arcsec":SCIENCE_EXCLUSION_ARCSEC,"target_gaia_source_excluded":EXPECTED_TARGET_GAIA_SOURCE_ID,"fit":"translation-only median; no clipping; diagnostic confidence"},
            "counts":{"common_same_gaia_references":len(refs),"poss_independent_references":len(p_ind),"dasch_independent_references":len(d_ind)},
            "measurement":{"median_poss_minus_gaia_east_arcsec":med_pe,"median_poss_minus_gaia_north_arcsec":med_pn,"median_dasch_minus_gaia_east_arcsec":med_de,"median_dasch_minus_gaia_north_arcsec":med_dn,"raw_poss_dasch_sep_arcsec":float(ptarget.separation(dtarget).arcsec),"corrected_poss_dasch_sep_arcsec":corrected_pd,"corrected_poss_to_v042_gaia_arcsec":p_gaia_corr,"corrected_dasch_to_v042_gaia_arcsec":d_gaia_corr,"poss_loo_residual_median_arcsec":None if not p_loo else float(np.median(p_loo)),"dasch_loo_residual_median_arcsec":None if not d_loo else float(np.median(d_loo))},
            "lessons_for_batch_automation":["primary common-reference registration first","if unavailable, independent per-archive Gaia translation allowed only with >=3 reciprocal refs and marked diagnostic","never use target or target Gaia source in fit","physical overlap interval remains authoritative","transport failure is not scientific evidence"],
            "detector_rerun":False,"science_image_pixels_read":False,"candidate_state_mutation":False,
            "next_stage":"Run deterministic same-tile morphology controls for both endpoints; if persistent-source association is supported and morphology is ordinary-star-like, close match 3 as not a transient survivor. Otherwise proceed to sensitivity-qualified recurrence."
        })
        print("\n"+"="*120)
        print("SPARSE LOCAL ASTROMETRY COMPLETE")
        print("="*120)
        ...
        print(f"Corrected to target Gaia: POSS={p_gaia_corr:.3f}\" DASCH={d_gaia_corr:.3f}\"")
        print("Report:",OUT / "order11_match3_sparse_astrometry_report_v044.json")
        return
    ...
    write_csv(OUT / "order11_match3_local_reference_audit_v044.csv", refs, ref_fields)
    write_json(OUT / "order11_match3_sparse_astrometry_report_v044.json", {
        "status": "COMPLETE",
        "analysis_kind": "order11_match3_independent_gaia_sparse_astrometry_v044",
        "classification": classification,
    ...
    print("Report:", OUT / "order11_match3_sparse_astrometry_report_v044.json")
    print("References:", OUT / "order11_match3_local_reference_audit_v044.csv")
