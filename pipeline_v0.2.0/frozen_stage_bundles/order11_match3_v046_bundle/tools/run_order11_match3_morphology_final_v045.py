...
OUT=ROOT/'results/order11_followup_match3_v045'
...
def main():
 print('='*120);print('ORDER 11 — MATCH 3 MORPHOLOGY + CONSOLIDATED DISPOSITION v045');print('='*120)
 ...
 if not V42.is_file() or not V44.is_file():raise RuntimeError('Missing v042/v044 prerequisite')
 ...
 v42=json.loads(V42.read_text(encoding='utf-8'));v44=json.loads(V44.read_text(encoding='utf-8'));acls=str(v44.get('classification'))
 ...
 elif 'DOES_NOT_SUPPORT_STRICT_RAW_COINCIDENCE' in acls:final='CLOSED_RAW_COINCIDENCE_NOT_SUPPORTED_AFTER_REGISTRATION'
 ...
 report={'status':'COMPLETE','analysis_kind':'order11_match3_morphology_final_v045','classification':final,'physical_overlap':{'start_utc':v42['measurement']['physical_overlap_start_utc'],'end_utc':v42['measurement']['physical_overlap_end_utc'],'actual_overlap_s':2700.0},'astrometry_classification':acls,'morphology':{'POSS':pm,'DASCH':dm},'candidate_adjudication_policy':{'path':str(ADJ_POLICY.relative_to(ROOT)),'sha256':policy_sha,'policy_id':policy.get('policy_id')},'science_image_pixels_read':True,'non_science_pixels_read':True,'detector_rerun':False,'candidate_state_mutation':False,'interpretation_boundary':'Closed means removed from the transient-survivor queue because a predeclared ordinary-source/astrometric explanation is sufficient; frozen detections and all evidence remain preserved. Survives means only that recurrence/sensitivity testing is still required, not that transience is established.'}
 OUT.mkdir(parents=True,exist_ok=True);wjson(OUT/'order11_match3_final_adjudication_v045.json',report);wjson(OUT/'candidate_adjudication_policy_v001.snapshot.json',policy);wcsv(OUT/'order11_match3_poss_morphology_controls_v045.csv',pa);wcsv(OUT/'order11_match3_dasch_morphology_controls_v045.csv',da)
 print('\nFINAL MATCH-3 DISPOSITION:',final);print('Report:',OUT/'order11_match3_final_adjudication_v045.json')
