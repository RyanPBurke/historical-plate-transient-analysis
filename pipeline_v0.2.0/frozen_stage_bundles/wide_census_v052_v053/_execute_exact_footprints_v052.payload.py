...
UA = 'historical-transient-pipeline/exact-footprint-v052'
...
ORDER BY plate_id, scan_id, solution_num, solution_id'''
    data = urllib.parse.urlencode({'REQUEST': 'doQuery', 'LANG': 'ADQL', 'FORMAT': 'csv', 'QUERY': query}).encode('utf-8')
    req = urllib.request.Request(TAP_URL, data=data, method='POST', headers={
        'User-Agent': UA, 'Accept': 'text/csv,text/plain,*/*', 'Content-Type': 'application/x-www-form-urlencoded'})
    with urllib.request.urlopen(req, timeout=180) as response:
        raw = response.read(); status = getattr(response, 'status', None); final_url = response.geturl()
    text = raw.decode('utf-8-sig', errors='strict')
    if '<html' in text[:500].lower() or '<votable' in text[:500].lower():
        raise RuntimeError('APPLAUSE TAP did not return CSV; first 300 chars: ' + repr(text[:300]))
    rows = list(csv.DictReader(io.StringIO(text)))
    if not rows:
        raise RuntimeError('APPLAUSE TAP returned zero solution rows')
    return rows, raw, status, final_url, query
...
def main():
    print('='*132); print('WIDE CENSUS — EXACT ARCHIVE-DERIVED SKY FOOTPRINT RESOLVER v052'); print('='*132)
    ...
    if not ap_cache.is_file():
        ...
        try:
            rows,raw,status,final_url,query=applause_tap_query(ap_ids); ap_cache.parent.mkdir(parents=True,exist_ok=True); ap_cache.write_bytes(raw)
            write_json(applause_meta_path(),{'http_status':status,'final_url':final_url,'query':query,'row_count':len(rows),'response_sha256':hashlib.sha256(raw).hexdigest()})
            print(f'APPLAUSE TAP: cached {len(rows)} solution rows for {len(ap_ids)} plates')
        except Exception as exc:
            n=mark_transport(cp,key,exc); print(f'APPLAUSE transport retry {n}/{MAX_TRANSPORT_ATTEMPTS}: {type(exc).__name__}: {exc}'); return 10
    remote=0
