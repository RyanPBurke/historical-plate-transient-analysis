...
Write-Host "Running v0.2.3 tests..."
& $python -m pytest tests -q
if ($LASTEXITCODE -ne 0) { throw "pytest failed" }

$migration = ".\research\EVIDENCE_INDEX_MIGRATION_v0.2.2_2026-08-20.json"
if (-not (Test-Path $migration)) {
    throw "v0.2.2 evidence migration record is missing; do not freeze v0.2.3."
}
...
Write-Host "Verifying evidence store before v0.2.3 control registration..."
& $exe verify-evidence --root .\evidence
if ($LASTEXITCODE -ne 0) { throw "evidence verification failed before v0.2.3 snapshot" }

Write-Host "Registering 5/5 mirror controls and preserved SkyView metadata..."
& $exe register-artifact --file $controls --kind skyview_dss_mirror_equivalence_controls --stage publication_freeze_v0.2.3 --snapshot
if ($LASTEXITCODE -ne 0) { throw "control-summary evidence registration failed" }
& $exe register-artifact --file $jar --kind skyview_control_reader_jar --stage publication_freeze_v0.2.3 --source-url "https://skyview.gsfc.nasa.gov/jar/skyview.jar"
if ($LASTEXITCODE -ne 0) { throw "SkyView JAR evidence registration failed" }
& $exe register-artifact --file $dss1b --kind skyview_dss1b_descriptor --stage publication_freeze_v0.2.3 --source-url "https://skyview.gsfc.nasa.gov/current/jar/surveys/xml/dss1b.xml.gz" --snapshot
if ($LASTEXITCODE -ne 0) { throw "DSS1B descriptor evidence registration failed" }
& $exe register-artifact --file $manifest --kind skyview_survey_manifest --stage publication_freeze_v0.2.3 --source-url "https://skyview.gsfc.nasa.gov/current/jar/surveys/survey.manifest" --snapshot
...
& $exe verify-evidence --root .\evidence
if ($LASTEXITCODE -ne 0) { throw "evidence verification failed after v0.2.3 control registration" }

$out = ".\research_snapshots\sub5_production_freeze_v0.2.3_2026-08-20"
...
& $exe --db .\state\publication_regression_v023.sqlite starglass `
  --manifest .\examples\regression_cases.csv `
  --plate bi05607 `
  --cache-dir .\cache\verified_starglass `
  --export .\results\publication_regression_v023.csv
if ($LASTEXITCODE -ne 0) { throw "regression execution failed" }

& $exe verify-regressions --results .\results\publication_regression_v023.csv
if ($LASTEXITCODE -ne 0) { throw "regression comparison failed" }

Write-Host ""
Write-Host "PUBLICATION FREEZE v0.2.3 PASSED."
Write-Host "SkyView raw-DSS fallback control basis: strict 5/5 pixel equivalence."
Write-Host "Next: run .\run_poss1_identity_preflight.ps1; no transient detection yet."
