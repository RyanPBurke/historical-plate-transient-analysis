...
OUTDIR = BASE / "order61_discovery_plate_audit_v028c"
...
OUT_ENDPOINT = OUTDIR / "order61_discovery_plate_endpoint_metrics_v028c.csv"
OUT_CONTROL = OUTDIR / "order61_discovery_plate_matched_controls_v028c.csv"
OUT_SUMMARY = OUTDIR / "order61_discovery_plate_candidate_summary_v028c.csv"
OUT_REPORT = OUTDIR / "order61_discovery_plate_audit_report_v028c.json"
...
def main():
    print("=" * 108)
    print("ORDER 61 — DISCOVERY-PLATE NATIVE PIXEL + SNR-MATCHED PEER AUDIT v028c")
    print("=" * 108)
    print(
    ...
    print(
        "Implementation amendments: optional 81x81 display cutouts are NaN-padded at native "
        "array/physical edges; prior morphology summary uses exact field "
        "endpoint_morphology_flag_count. Quantitative morphology and peer policy are unchanged."
    )
    print()
    ...
    for rank in ACTIVE_RANKS:
        ...
        summary = {
            ...
            "prior_morphology_flag_count": int(pm["endpoint_morphology_flag_count"]),
    ...
    report = {
        "status": "COMPLETE",
        "analysis_kind": "order61_discovery_plate_native_pixel_snr_matched_peer_audit_v028c",
        "guards": guards,
        "implementation_amendments": [
            {
                ...
                "change": (
                    "optional 81x81 display extraction NaN-pads unavailable edge pixels "
            ...
            {
                "prior_worker": "run_order61_discovery_plate_peer_audit_v028b.py",
                "failure_stage": (
                    "summary assembly after rank-11 endpoint calculations, before any "
                    "endpoint/control/summary CSV or completed report was written"
                ),
                "change": (
                    "use exact prior morphology CSV field endpoint_morphology_flag_count "
                    "instead of nonexistent morphology_flag_count"
                ),
                "prior_rank11_quantitative_calculations_occurred_in_memory": True,
                "prior_rank11_quantitative_results_printed": False,
                "prior_completed_quantitative_csv_or_report_written": False,
                "partial_audit_npy_may_have_been_written": True,
                "v028c_uses_distinct_audit_directory": True,
                "scientific_morphology_function_changed": False,
                "peer_selection_changed": False,
                "snr_matching_changed": False,
                "candidate_gate_changed": False
