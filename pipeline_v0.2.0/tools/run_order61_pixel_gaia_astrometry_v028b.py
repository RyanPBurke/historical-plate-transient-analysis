...
def skyoff(a,b):
    """
    Tangent-plane offset after explicitly stripping non-positional frame
    metadata (notably obstime carried by propagated Gaia coordinates).

    v028 produced no POSS pixel measurements because Astropy refused
    spherical_offsets_to() between otherwise-ICRS coordinates with
    non-matching frame attributes.  This changes representation only;
    RA/Dec positions and all scientific selection/measurement thresholds
    are unchanged.
    """
    ai = a.icrs
    bi = b.icrs
    aa = SkyCoord(
        ra=float(ai.ra.deg) * u.deg,
        dec=float(ai.dec.deg) * u.deg,
        frame="icrs",
    )
    bb = SkyCoord(
        ra=float(bi.ra.deg) * u.deg,
        dec=float(bi.dec.deg) * u.deg,
        frame="icrs",
    )
    e,n = aa.spherical_offsets_to(bb)
...
def main():
    print("="*102);print("ORDER 61 — ORDINARY-GAIA-STAR NATIVE-PIXEL LOCAL ASTROMETRY v028b");print("="*102)
    print("Implementation amendment: strip non-positional SkyCoord frame metadata before tangent-plane offsets; scientific policy unchanged.")
