...
MANIFEST=FREEZE/"freeze_manifest_v094r3b.json"
...
RESULT=ROOT/"results"/"applause_dr4_historical_geometry_validation_v094r3b"
...
def self_test():
    ...
    print("v094r3b historical-geometry self-test PASS")
    return 0
...
def preflight():
    ...
    print("v094r3b FREEZE PREFLIGHT PASS")
...
def run():
    ...
    for oi,r in enumerate(overlap,1):
            ...
            for true_t in true_times:
                ...
                for D in DISTANCES_KM:
                    ...
                    for nA,nB in chosen:
                        ...
                        ok=False
                        if rr["generator_min_xt"]<=GEN_ARCSEC and rr["final"][1] is not None:
                            td,det=rr["final"]
        ...
        if not nominal_ok:
            cls="GEOMETRY_UNRESOLVED_REFERENCE_COVERAGE"
        elif any(x in reasons for x in ("APPLAUSE_HORIZONTAL_DATUM_ACCURACY_UNDOCUMENTED","APPLAUSE_ELEVATION_DATUM_UNDOCUMENTED")):
            cls="GEOMETRY_UNRESOLVED_SITE_METADATA"
        elif pub_fail>0 or (synth_observable>0 and synth_recovered<synth_observable):
            cls="GEOMETRY_UNRESOLVED_PRECISION"
        else:
            cls="GEOMETRY_SUPPORTED_FOR_SYNTHETIC_TEST"
        agg[cls]+=1
        ...
        if oi%50==0:log(f"v094r3b geometry/synthetic validation: {oi}/1081")

    RESULT.mkdir(parents=True,exist_ok=True)
    csvp=RESULT/"opportunity_geometry_validation_v094r3b.csv"
    with csvp.open("w",encoding="utf-8",newline="") as f:
        w=csv.DictWriter(f,fieldnames=list(opp_rows[0].keys()));w.writeheader();w.writerows(opp_rows)
    report={
      "status":"COMPLETE",
      "analysis_kind":"applause_dr4_historical_geometry_validation_v094r3b",
    ...
    rp=RESULT/"applause_dr4_historical_geometry_validation_v094r3b.json"
    report["output_hashes"]={csvp.name:sha(csvp)}
    rp.write_text(json.dumps(report,indent=2,sort_keys=True)+"\n",encoding="utf-8")
    man=RESULT/"v094r3b_output_manifest.sha256"
    man.write_text(f"{sha(csvp)}  {csvp.name}\n{sha(rp)}  {rp.name}\n",encoding="utf-8")

    print("\n"+"="*108)
    print("v094r3b HISTORICAL GEOMETRY + OBSERVABLE SYNTHETIC VALIDATION COMPLETE")
    print("="*108)
