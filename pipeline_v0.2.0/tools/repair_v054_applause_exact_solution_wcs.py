...
PAYLOAD = ROOT / "tools" / "_preflight_wide_heavy_detector_v054_schemafix2.payload.py"

EXPECTED_OLD_NORMALIZED_SHA = "e7b9de075cd56f87e88ae55250a76b4109d1b3d3e618c783c20719e408a0ea80"
EXPECTED_NEW_NORMALIZED_SHA = "d5f23234ea2a5af8cabb12875df4199752ec1875052e902e0b60aa8afa3b2c1e"
...
def main():
    print("=" * 132)
    print("HEAVY PREFLIGHT v054 — EXACT APPLAUSE SOLUTION-ID WCS REPAIR 2")
    print("=" * 132)
    ...
    if current_norm not in (
        ...
        raise RuntimeError(
            ...
            "schema-fix-1 and schema-fix-2 sources: " + current_norm
        )

    if payload_norm != EXPECTED_NEW_NORMALIZED_SHA:
        raise RuntimeError(
            "REFUSING: schema-fix-2 payload normalized SHA mismatch: "
            + payload_norm
        )

    stamp = dt.datetime.now(dt.timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    backup = ROOT / "patch_backups" / f"pre_v054_schemafix2_{stamp}"
    ...
    if installed != EXPECTED_NEW_NORMALIZED_SHA:
        raise RuntimeError(
            "Installed v054 schema-fix-2 source hash mismatch: " + installed
        )
