...
def main():
    ...
    print("="*132)
    print("WIDE CENSUS — CACHED GAIA DR3 ACQUISITION v064 (RESUME PERFORMANCE FIX 1)")
    print("="*132)
    ...
    # FIX1:
    # The original worker called summarize(plan,hplan) before EVERY plan row.
    # On a resumed run with thousands of cached cells this produced an O(N^2)
    # cache-validation loop before reaching the first unfinished cell.
    #
    # Perform one complete integrity scan at startup, build an in-memory set of
    # completed base-query IDs, then refresh the full manifest only at bounded
    # checkpoints and at finalization. No cache semantics or science logic changes.
    print("Scanning and verifying existing Gaia cache once ...", flush=True)
    rows0,c0,l0,r0,h0,hr0=summarize(plan,hplan)
    completed_base={
        int(r["global_query_index"])
        for r in rows0
        if r.get("status")=="COMPLETE"
    }
    print(
        f"RESUME CACHE: ordinary base {c0}/{len(plan)} complete | "
        f"resolved leaves {l0} | HPM {h0}/{len(hplan)}",
        flush=True,
    )
    ...
    for i,r in enumerate(plan,1):
        gid=int(r["global_query_index"])
        if gid in completed_base:
            continue
        ...
        print(
            f"[ordinary {i}/{len(plan)}] global cell {r['global_query_index']} "
            f"consumers={r['consumer_pair_count']}",
            flush=True
        )

        ordinary_leaf(
            r,base_bounds(r),"",0,r["consumer_pair_indices"],counter
        )
        completed_base.add(gid)
        newly_completed_since_checkpoint+=1

        if newly_completed_since_checkpoint>=20:
            rows2,c2,l2,r2,h2,hr2=summarize(plan,hplan)
            write_json(STATE,{
                "status":"IN_PROGRESS",
                "ordinary_base_complete":c2,
                "ordinary_base_total":len(plan),
                "ordinary_leaf_complete":l2,
                "ordinary_cached_rows":r2,
                "hpm_complete":h2,
                "hpm_total":len(hplan),
                "new_network_calls_this_invocation":counter["new"],
                "updated_at_utc":datetime.now(timezone.utc).isoformat(),
                "resume_performance_fix":"v064_fix1",
            })
            print(
                f"CHECKPOINT ordinary base {c2}/{len(plan)} | "
                f"HPM {h2}/{len(hplan)}",
                flush=True
            )
            newly_completed_since_checkpoint=0
    ...
    if not stopped:
        ...
        for i,r in enumerate(hplan,1):
            ...
            if counter["new"]>before:
                newly_hpm_since_checkpoint+=1

            if newly_hpm_since_checkpoint>=5:
    ...
    write_csv(MANIFEST,rows,[
        ...
        "consumer_pair_indices","status","resolved_leaf_queries",
        "cached_rows_across_leaves"
    ])
    ...
    rep={
        ...
        "resume_performance_fix":{
            "version":"v064_fix1",
            "change":"one startup cache scan plus bounded checkpoint scans; removed per-plan-row full-cache rescans",
            "science_or_transport_semantics_changed":False,
        },
    ...
    print("Resume performance fix: v064_fix1")
