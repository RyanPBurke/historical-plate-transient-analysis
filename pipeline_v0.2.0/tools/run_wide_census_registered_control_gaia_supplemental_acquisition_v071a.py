...
OUTDIR=ROOT/"results/wide_census_registered_control_gaia_supplemental_acquisition_v071a"
CACHE=OUTDIR/"cache"
OCACHE=CACHE/"ordinary"
HCACHE=CACHE/"hpm"
STATE=OUTDIR/"state_v071a.json"
REPORT=OUTDIR/"wide_census_registered_control_gaia_supplemental_acquisition_v071a.json"
MANIFEST=OUTDIR/"wide_census_registered_control_gaia_supplemental_manifest_v071a.csv"
...
def main():
    ...
    if args.workers not in (1,2,3,4,5,6,7,8):
        raise RuntimeError("REFUSING: v071a supports only 1 to 8 workers; default remains 2 and operational maximum is 8")

    print("="*136)
    print("WIDE CENSUS — REGISTERED-CONTROL SUPPLEMENTAL GAIA DR3 ACQUISITION v071a")
    print("="*136)
    ...
    print("Scanning/verifying existing v071a cache once ...",flush=True)
    ...
    with ThreadPoolExecutor(max_workers=args.workers,thread_name_prefix="gaia-v071a") as ex:
    ...
    if args.max_new is not None and completed_this_run>=args.max_new and len(pending)>completed_this_run:
        ...
        with ThreadPoolExecutor(max_workers=args.workers,thread_name_prefix="gaia-v071aa-hpm") as ex:
    ...
    log("\nFINAL INTEGRITY SCAN: verifying v071a cache ...")
    manifest,done,leaves,rows,cb,hdone,hrows,hcb=scan_all(plan,hplan)
    ...
    rep={
        "status":status,
        "analysis_kind":"wide_census_registered_control_gaia_supplemental_acquisition_v071a",
        "updated_at_utc":datetime.now(timezone.utc).isoformat(),
        "guards":{
