...
CONTRACT=ROOT/'research'/'prospective_freezes'/'v094s_source_free_opportunity_population_repair_contract.json'
PROV=ROOT/'research'/'prospective_freezes'/'v094s_source_free_opportunity_population_parent_provenance.json'
EXPECTED='133e4c1ebd0f35cdda0e6ffa4a51d1136f4dc959355cb2fd58174959ab4c15eb'
RESULT=ROOT/'results'/'applause_dr4_source_free_opportunity_population_repair_v094s'
...
def self_test():
 ...
 p1=[(0,0),(1,0),(1,1),(0,1)];p2=[(.5,.5),(1.5,.5),(1.5,1.5),(.5,1.5)];assert poly_intersection_metrics(p1,p2)['positive']
 print('v094s source-free population repair self-test PASS');return 0
def run():
 if not CONTRACT.is_file() or sha(CONTRACT)!=EXPECTED:raise SystemExit('v094s contract SHA mismatch')
 ...
 with outp.open('w',encoding='utf-8',newline='') as f:
  ...
  for n,(key,pr) in enumerate(sorted(pairs.items()),1):
   ...
   m=poly_intersection_metrics(ea['poly'],eb['poly'])
   if m is None:raise SystemExit(f'Old intersection replay mechanical HOLD for pair {key}')
   oldpos=bool(m['positive']);d=hav(ea['lat'],ea['lon'],eb['lat'],eb['lon']);bb=baseline_band(d)
 ...
 report={'status':'COMPLETE','analysis_kind':'applause_dr4_source_free_opportunity_population_repair_v094s','contract_sha256':EXPECTED,'parent_provenance_sha256':sha(PROV),'master_rows':master_rows,'eligible_exposures':len(exposures),'full_population_timing_interval_mismatches':timing_replay_mismatch,'continuous_fragment_records':len(frags),'distinct_site_temporal_pairs_le15min':len(pairs),'exclusive_timing_bins_all':dict(allbins),'cumulative_timing_gates_all':{'LE5MIN_INCLUDING_OVERLAP':allbins['OVERLAP']+allbins['GT0_LE5MIN'],'LE10MIN_INCLUDING_OVERLAP':allbins['OVERLAP']+allbins['GT0_LE5MIN']+allbins['GT5_LE10MIN'],'LE15MIN_INCLUDING_OVERLAP':len(pairs)},'old_common_positive_replay':{'rows':sum(oldbins.values()),'exclusive_timing_bins':dict(oldbins),'old_le5_plan_keys':len(oldplan),'missing_old_le5_keys':len(missing),'extra_old_le5_keys':len(extra)},'newly_recovered_no_common_intersection':{'rows':sum(newbins.values()),'exclusive_timing_bins':dict(newbins),'overlap_rows':newbins['OVERLAP']},'same_site_temporal_context_pairs':len(same_site),'fragment_reconstruction_status_counts':dict(fragment_status),'exposure_preflight_holds':dict(holds),'site_baseline_band_counts_diagnostic_only':dict(baseline),'epoch_counts_all':dict(epochs),'top_site_pairs_all':sitepairs.most_common(30),'guards':{'source_catalog_reads':0,'candidate_identity_inspection':0,'private_candidate_map_reads':0,'network':0,'pixels':0,'registration':0,'real_source_pairing':0},'next_stage':'Acquire/freeze elevations for any newly relevant plates and validate corrected time-dependent geometry/observable synthetic recovery on newly recovered OVERLAP opportunities.','output_hashes':{outp.name:sha(outp)}}
 rp=RESULT/'applause_dr4_source_free_opportunity_population_repair_v094s.json';rp.write_text(json.dumps(report,indent=2,sort_keys=True)+'\n',encoding='utf-8');man=RESULT/'v094s_output_manifest.sha256';man.write_text(f"{sha(outp)}  {outp.name}\n{sha(rp)}  {rp.name}\n",encoding='utf-8')
 print('\n'+'='*108);print('v094s SOURCE-FREE CROSS-SITE OPPORTUNITY POPULATION REPAIR COMPLETE');print('='*108)
 print(f'Master exposure rows:                         {master_rows:,}');print(f'Eligible exposures:                           {len(exposures):,}');print(f'Full-pop timing interval mismatches:           {timing_replay_mismatch}');print(f'Distinct-site temporal pairs <=15 min:         {len(pairs):,}');print(f'All exclusive timing bins:                     {dict(allbins)}');print(f'Old common-positive replay rows/bins:          {sum(oldbins.values()):,} / {dict(oldbins)}');print(f'Old <=5 stable-key replay missing/extra:       {len(missing)} / {len(extra)}');print(f'New no-common-intersection rows:               {sum(newbins.values()):,}');print(f'New no-common-intersection timing bins:        {dict(newbins)}');print(f'NEW OVERLAP opportunities recovered:           {newbins["OVERLAP"]:,}');print('Source catalogues / identities / network:      0 / 0 / 0');print('STOP: validate geometry on newly recovered OVERLAP opportunities before any real-source matcher.');return 0
def main():
