...
def pair_row():
    ...
    if len(hits) != 1:
        raise RuntimeError(f"REFUSING: order61 row count={len(hits)}")
    r = hits[0]
    for k, v in {
        ...
        if r.get(k) != v:
            raise RuntimeError(f"REFUSING: order61 {k}={r.get(k)!r}, expected {v!r}")
...
def validate_reference(dss_world):
    ...
    if (fw, fh) != (14000, 13999):
        raise RuntimeError(f"REFUSING: unexpected XE520 full shape {fw}x{fh}")
...
def main():
    print("="*88)
    print("ORDER 61 â€” RESUMABLE WHOLE-FOOTPRINT NATIVE DETECTOR EXECUTION")
    print("="*88)
    ...
    pcsv=RESULT/"order61_poss_native_candidates.csv"
    dcsv=RESULT/"order61_dasch_native_candidates.csv"
    mcsv=RESULT/"order61_raw_coincidences.csv"
    ...
    report={
        "status":"COMPLETE","run_kind":"order61_whole_footprint_native_tile_frozen_detector",
        ...
        "interpretation":"Complete deterministic native-pixel tile execution for the full order-61 POSS footprint. Raw coincidences are not transient classifications; downstream static-source, morphology, PSF, saturation, registration, controls and injection-recovery remain required.",
    }
    rp=RESULT/"order61_whole_pair_report.json"; write_json(rp,report)
