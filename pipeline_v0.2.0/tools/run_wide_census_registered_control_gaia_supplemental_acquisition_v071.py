...
OUTDIR=ROOT/"results/wide_census_registered_control_gaia_supplemental_acquisition_v071"
CACHE=OUTDIR/"cache"
OCACHE=CACHE/"ordinary"
HCACHE=CACHE/"hpm"
STATE=OUTDIR/"state_v071.json"
REPORT=OUTDIR/"wide_census_registered_control_gaia_supplemental_acquisition_v071.json"
MANIFEST=OUTDIR/"wide_census_registered_control_gaia_supplemental_manifest_v071.csv"
...
def main():
    ...
    if args.workers not in (1,2,3,4,5,6,7,8):
        raise RuntimeError("REFUSING: v071 supports only 1 to 8 workers; default remains 2 and operational maximum is 8")

    print("="*136)
    print("WIDE CENSUS — REGISTERED-CONTROL SUPPLEMENTAL GAIA DR3 ACQUISITION v071")
    print("="*136)
    ...
    if modes.get("FULL_NEW_BASE_CELL_CONTROL")!=EXPECTED_NEW_FULL or modes.get("MARGIN_ANNULUS_EXISTING_LEAF")!=EXPECTED_ANNULUS:
    ...
    print("Scanning/verifying existing v071 cache once ...",flush=True)
    ...
    with ThreadPoolExecutor(max_workers=args.workers,thread_name_prefix="gaia-v071") as ex:
    ...
    if args.max_new is not None and completed_this_run>=args.max_new and len(pending)>completed_this_run:
        ...
        with ThreadPoolExecutor(max_workers=args.workers,thread_name_prefix="gaia-v071-hpm") as ex:
    ...
    log("\nFINAL INTEGRITY SCAN: verifying v071 cache ...")
    manifest,done,leaves,rows,cb,hdone,hrows,hcb=scan_all(plan,hplan)
    ...
    rep={
        "status":status,
        "analysis_kind":"wide_census_registered_control_gaia_supplemental_acquisition_v071",
        "updated_at_utc":datetime.now(timezone.utc).isoformat(),
        "guards":{
