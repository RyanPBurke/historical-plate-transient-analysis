...
def parse_exposure(rank, target_ra, target_dec, row):
    ...
    # APASS is preferred by daschlab; ATLAS is a fallback when APASS
    # calibration is unavailable for a specific exposure.
    if has_apass:
        selected = "apass"
    elif has_atlas:
        selected = "atlas"
    ...
    return {
        ...
        "resultIdApass": str(getv(row, "resultIdApass") or "").strip(),
        "resultIdAtlas": str(getv(row, "resultIdAtlas") or "").strip(),
...
def main():
    print("=" * 94)
    print("ORDER 61 — AI44092 PLATEPHOT CONTEXT + INDEPENDENT EXPOSURE CENSUS")
    print("=" * 94)
    print(
    ...
    for rank in EXPECTED_RANKS:
        ...
        pair_exps = [
            ...
            and e["selected_refcat_for_platephot"] is not None
        ]
        ...
        census[rank] = {
            ...
            "ai44092_photometric_solutions": len(pair_exps),
        ...
        print(
            ...
            f"ai44092-solutions={len(pair_exps)}",
            flush=True,
        )
    ...
    for rank in EXPECTED_RANKS:
        ...
        if not pair_exps:
            raise RuntimeError(
                f"REFUSING: strict #{rank:02d} queryexps has no calibrated "
                "ai44092 exposure/solution"
            )

        all_current_sources = []
        calls = 0

        # Query every calibrated ai44092 solution that queryexps says covers
        # this sky position. This follows the DASCH multiple-exposure guidance.
        seen_call = set()
        ...
        for e in pair_exps:
            key = (
                e["plate_id"],
                e["solnum"],
                e["selected_refcat_for_platephot"],
            )
            if key in seen_call:
                continue
            seen_call.add(key)
            ...
            # RA/Dec must be locatable in the raw extract.
            ...
            if not (
                ...
                raise RuntimeError(
                    f"REFUSING: strict #{rank:02d} platephot response has "
                    f"no usable RA/Dec columns; header={header}"
                )
            ...
            for row in rows:
                ...
                if src["sep_dasch_endpoint_arcsec"] <= SOURCE_AUDIT_RADIUS_ARCSEC:
    ...
    report = {
        ...
        "exposure_selection_contract": {
            ...
            "has_apass_phot": "finite limMagApass on an imaged exposure",
            "has_atlas_phot": "finite limMagAtlas on an imaged exposure",
