...
def save_png(p,a,cx,cy,title,hp=False):
 ...
 if hp:
  fin=np.isfinite(q);fill=float(np.nanmedian(q[fin])) if np.any(fin) else 0;work=np.where(fin,q,fill);im=work-gaussian_filter(work,BGSIG);s=rsig(im);clip=HPCLIP*s if s is not None else max(1.0,float(np.std(im)));im=np.clip(im,-clip,clip);vmin,vmax=-clip,clip
 else:im,lo,hi=rawnorm(q);vmin,vmax=0,1
 fig=plt.figure(figsize=(5,5),dpi=140);ax=fig.add_axes([.04,.04,.92,.88]);ax.imshow(im,origin='lower',cmap='gray',vmin=vmin,vmax=vmax,interpolation='nearest');arm=max(4,min(im.shape)*.08);gap=max(2,min(im.shape)*.025);ax.plot([cx-arm,cx-gap],[cy,cy],linewidth=1);ax.plot([cx+gap,cx+arm],[cy,cy],linewidth=1);ax.plot([cx,cx],[cy-arm,cy-gap],linewidth=1);ax.plot([cx,cx],[cy+gap,cy+arm],linewidth=1);ax.set_title(title,fontsize=9);ax.set_xticks([]);ax.set_yticks([]);fig.savefig(p);plt.close(fig)
...
def main():
 print('='*120);print('PAIR 17 — STANDARDIZED MANUAL DOSSIERS v083');print('='*120)
 ...
 for i,panel in enumerate(selected,1):
  ...
  if science:
   obs=panel['observatory'];path=science_paths[obs];tr=tri[rid];ep=SCI[obs]['endpoint'];cx=float(tr[f'{ep}_x_global']);cy=float(tr[f'{ep}_y_global']);m={}
  else:
   aq=acq.get((sid,pid));
