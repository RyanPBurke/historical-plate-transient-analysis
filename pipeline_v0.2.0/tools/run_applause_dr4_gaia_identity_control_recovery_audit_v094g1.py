...
CONTRACT_REL = Path('pipeline_v0.2.0/research/prospective_freezes/applause_dr4_gaia_identity_control_recovery_operational_amendment_v094g1.json')
PROV_REL = Path('pipeline_v0.2.0/research/prospective_freezes/applause_dr4_gaia_identity_control_recovery_operational_provenance_v094g1.json')
RUNNER_REL = Path('pipeline_v0.2.0/tools/run_applause_dr4_gaia_identity_control_recovery_audit_v094g1.py')
INV_REL = Path('pipeline_v0.2.0/research/prospective_freezes/applause_dr4_v094c_source_cache_inventory_v094e.csv')
PARENT_COMMIT = 'edb7663eac6d6e57cefecbc7e18ee5a553238484'
EXPECTED_SAMPLE_SHA256 = 'd2b39fdbad4af36934be356b894affe227c4e211ac5e113a90609dc5c19b9c33'
SCIENCE_INITIAL_BATCH = 250
SCIENCE_MIN_BATCH = 25
...
def verify_git(repo: Path, freeze: str):
    ...
    if subprocess.run(['git','-C',str(repo),'merge-base','--is-ancestor',PARENT_COMMIT,freeze]).returncode != 0:
        raise RuntimeError('v094g1 freeze does not descend from frozen v094g parent')
...
def tap_table(query: str, raw: Path, label: str, expected_cols, maxrec=500000, attempts=3):
...
def _source_batch_key(group):
    payload = ','.join(map(str, group)).encode('ascii')
    return hashlib.sha256(payload).hexdigest()[:20]


def _fetch_science_group(project: Path, group, depth=0):
    work = project/'work'/'applause_dr4_gaia_identity_control_recovery_audit_v094g1'/'tap_raw'/'science_source_calib'
    group = list(group)
    key = _source_batch_key(group)
    raw = work/f'science_n{len(group):04d}_{key}.vot'
    q = f"SELECT {','.join(SCIENCE_FIELDS)} FROM applause_dr4.source_calib WHERE source_id IN ({','.join(map(str,group))})"
    label = f'science source_calib n={len(group)} key={key[:8]}'
    try:
        tbl, status = tap_table(q, raw, label, SCIENCE_FIELDS, maxrec=max(1000, len(group)+10), attempts=2)
        return [tbl], [{'group_size':len(group),'key':key,'status':status,'depth':depth}]
    except Exception as e:
        if len(group) <= SCIENCE_MIN_BATCH:
            raise RuntimeError(f'science source_calib transport failed at minimum batch n={len(group)} key={key}: {e}') from e
        mid = len(group)//2
        left, right = group[:mid], group[mid:]
        print(f'{label}: adaptive split {len(group)} -> {len(left)} + {len(right)} after {type(e).__name__}: {e}', flush=True)
        lt, lr = _fetch_science_group(project, left, depth+1)
        rt, rr = _fetch_science_group(project, right, depth+1)
        return lt+rt, [{'group_size':len(group),'key':key,'status':'SPLIT','depth':depth,'error_class':type(e).__name__}] + lr + rr
...
def acquire_science_metadata(project: Path, source_ids):
    ...
    meta = {}
    audit=[]
    initial_groups=[ids[i:i+SCIENCE_INITIAL_BATCH] for i in range(0,len(ids),SCIENCE_INITIAL_BATCH)]
    for gi, group in enumerate(initial_groups,1):
        print(f'science acquisition initial group {gi}/{len(initial_groups)}: {len(group)} ids', flush=True)
        tables, recs = _fetch_science_group(project, group)
        audit.extend(recs)
        for tbl in tables:
            for r in tbl:
                ...
                if sid is None:
                    continue
                if sid not in set(group):
                    raise RuntimeError(f'unrequested source_id returned by source_calib: {sid}')
                if sid in meta:
    ...
    state = project/'work'/'applause_dr4_gaia_identity_control_recovery_audit_v094g1'/'state'
    ...
    (state/'science_source_calib_acquisition.json').write_text(json.dumps({
        ...
        'missing_source_ids_count': len(missing), 'initial_batch_size':SCIENCE_INITIAL_BATCH,
        'minimum_adaptive_batch_size':SCIENCE_MIN_BATCH,'initial_groups':len(initial_groups),
    ...
    return meta, {'requested':len(ids),'returned':len(meta),'initial_groups':len(initial_groups),
                  'completed_leaf_queries':sum(r.get('status') in {'COMPLETE','CACHE_REUSED'} for r in audit),
                  'adaptive_splits':sum(r.get('status')=='SPLIT' for r in audit)}
...
def make_control_query_batches(by_scan, max_pairs=150, max_clauses=8):
    clauses = []
    for sid in sorted(by_scan):
        gids = sorted(by_scan[sid])
        for start in range(0, len(gids), 100):
            chunk = gids[start:start+100]
...
def acquire_control_recovery(project: Path, by_scan):
    work = project/'work'/'applause_dr4_gaia_identity_control_recovery_audit_v094g1'/'tap_raw'/'control_source_calib'
...
def acquire_plate_metadata(project: Path, plate_ids):
    work = project/'work'/'applause_dr4_gaia_identity_control_recovery_audit_v094g1'/'tap_raw'/'plate_metadata'
...
def save_internal_sample(project, sample):
    state=project/'work'/'applause_dr4_gaia_identity_control_recovery_audit_v094g1'/'state';state.mkdir(parents=True,exist_ok=True)
    p=state/'deterministic_sample_internal_v094g1.jsonl'
...
def main():
    ...
    if contract.get('status')!='PROSPECTIVE_OPERATIONAL_ACQUISITION_AMENDMENT_V094G1' or prov.get('status')!='FROZEN_V094G1_OPERATIONAL_AMENDMENT_PROVENANCE':raise RuntimeError('v094g1 contract/provenance status mismatch')
    verify_parent_inputs(project,repo,prov);print('Frozen v094g1 parent inputs verified',flush=True)
    mod=load_v094c_module(project,prov);plate_scans,scan_polys=load_geometry(project,prov)
    print('Replaying frozen v094c mechanics and reconstructing unchanged deterministic v094g sample...',flush=True)
    ...
    if internal_sha != EXPECTED_SAMPLE_SHA256:
        raise RuntimeError(f'deterministic sample hash changed under operational amendment: {internal_sha} expected {EXPECTED_SAMPLE_SHA256}')
    print(f'Deterministic sample reproduced exactly: {len(sample):,} rows; sha256={internal_sha}',flush=True)
    ...
    print('\n'+'='*104);print('v094g1 GAIA IDENTITY / CONTROL RECOVERY AUDIT COMPLETE');print('='*104)
