...
def refresh_dasch_package(plate, frozen_pkg):
    """Refresh ephemeral DR7 download credentials without re-selecting geometry."""
    last_exc = None
    payload = json.dumps({
        "plate_id": str(plate).strip().lower(),
        "binning": 1,
    }).encode("utf-8")

    for attempt in range(1, DASCH_PACKAGE_REFRESH_ATTEMPTS + 1):
        try:
            # StarGlass may cache mosaic_package responses. Cache-bust the
            # request and explicitly require substantial lifetime remaining
            # on the returned AWS pre-signed URL before using it.
            api_url = (
                DASCH_MOSAIC_API
                + f"?transport_refresh={time.time_ns()}_{attempt}"
            )
            ...
            url = str(fresh.get("baseFitsUrl") or "")
            size = int(fresh.get("baseFitsSize") or 0)
            if not url:
                raise RuntimeError("fresh DASCH mosaic_package returned no baseFitsUrl")
            if size < 10_000_000:
                raise RuntimeError(
                    f"fresh DASCH mosaic_package returned implausible baseFitsSize={size}"
                )

            remaining_s = signed_url_remaining_seconds(url)
            if remaining_s is None:
                raise RuntimeError(
                    "fresh DASCH mosaic_package returned signed URL without parseable expiry"
                )

            # A 900-second signed URL does NOT need 720 seconds remaining to
            # be scientifically valid. It only needs enough lifetime to open
            # and read the next native section; v056 already self-heals on a
            # later 403 by refreshing the transport without changing geometry.
            #
            # If the application cache is still serving a credential at the
            # actual expiry boundary, wait for that cached object to roll over
            # rather than burning the persistent science-tile retry budget.
            if remaining_s < DASCH_MIN_SIGNED_URL_VALIDITY_S:
                if remaining_s > 0:
                    wait_s = min(
                        DASCH_MIN_SIGNED_URL_VALIDITY_S,
                        max(3.0, remaining_s + 3.0),
                    )
                else:
                    wait_s = DASCH_EXPIRED_CACHE_RETRY_SLEEP_S
                print(
                    f"  DASCH {plate}: cached signed URL has only "
                    f"{remaining_s:.1f}s remaining; waiting {wait_s:.1f}s "
                    f"for credential rollover (refresh attempt {attempt}/"
                    f"{DASCH_PACKAGE_REFRESH_ATTEMPTS})",
                    flush=True,
                )
                time.sleep(wait_s)
                last_exc = RuntimeError(
                    f"cached DASCH signed URL at expiry boundary: remaining={remaining_s:.1f}s"
                )
                continue

            # This is the critical science guard: the refreshed package is
            # allowed to change transport credentials only, not the frozen
            # WCS/rotation/mosaic geometry.
            frozen_sig = dasch_geometry_sig(frozen_pkg)
            fresh_sig = dasch_geometry_sig(fresh)
            if fresh_sig != frozen_sig:
                raise RuntimeError(
                    "REFUSING: fresh DASCH package geometry differs from frozen v052 geometry"
                )

            frozen_size = int(frozen_pkg.get("baseFitsSize") or 0)
            if frozen_size and size != frozen_size:
                raise RuntimeError(
                    f"REFUSING: fresh DASCH baseFitsSize {size} != frozen {frozen_size}"
                )

            parsed = urllib.parse.urlsplit(url)
            stable_path = urllib.parse.urlunsplit(
                (parsed.scheme, parsed.netloc, parsed.path, "", "")
            )
            plate_token = f"/plates/{str(plate).strip().lower()}/"
            if plate_token not in parsed.path.lower():
                raise RuntimeError(
                    f"REFUSING: refreshed DASCH URL path does not identify plate {plate}: "
                    f"{parsed.path}"
                )
            ...
            return {
                "package": fresh,
                "signed_url": url,
                "stable_object_url": stable_path,
                "package_response_sha256": hashlib.sha256(raw).hexdigest(),
                "geometry_sha256": fresh_sig,
                "baseFitsSize": size,
            ...
            last_exc = exc
            if attempt < DASCH_PACKAGE_REFRESH_ATTEMPTS:
                time.sleep(min(8, 2 * attempt))

    raise RuntimeError(
        f"DASCH mosaic_package refresh failed after "
        f"{DASCH_PACKAGE_REFRESH_ATTEMPTS} attempts for {plate}: "
...
def open_endpoint(endpoint):
    ...
    if kind == "DASCH":
        ...
        # baseFitsUrl is an ephemeral AWS pre-signed URL (15-minute lifetime),
        # so never reuse the URL frozen in an earlier metadata/preflight stage.
        # Refresh transport immediately before opening the endpoint, while
        # requiring the fresh package geometry signature to match v052 exactly.
        ...
        return {
            ...
            "transport_method": "FRESH_DASCH_MOSAIC_PACKAGE_PRESIGNED_URL_DIRECT_HTTPFILESYSTEM",
            "transport_refresh": {
                "package_response_sha256": transport["package_response_sha256"],
...
def main():
    print("=" * 132)
    print("WIDE CENSUS — DISK-BOUNDED RESUMABLE FROZEN DETECTOR EXECUTION v056 (DASCH CACHED-CREDENTIAL ROLLOVER FIX 5)")
    print("=" * 132)
    ...
    for endpoint_key in order:
        ...
        try:
            ...
            # If the endpoint cannot be opened even after refresh_dasch_package
            # has exhausted its own fresh-credential attempts, retain this as
