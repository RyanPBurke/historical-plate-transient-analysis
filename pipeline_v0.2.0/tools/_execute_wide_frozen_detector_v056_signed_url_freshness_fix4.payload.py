...
DASCH_PACKAGE_REFRESH_ATTEMPTS = 6
DASCH_MIN_SIGNED_URL_VALIDITY_S = 720
DASCH_MAX_INLINE_TRANSPORT_REFRESHES = 3
    ...
    for attempt in range(1, DASCH_PACKAGE_REFRESH_ATTEMPTS + 1):
        try:
            ...
            if remaining_s < DASCH_MIN_SIGNED_URL_VALIDITY_S:
                raise RuntimeError(
                    f"fresh DASCH mosaic_package returned near-expiry signed URL: "
                    f"remaining={remaining_s:.1f}s < required "
                    f"{DASCH_MIN_SIGNED_URL_VALIDITY_S}s"
                )
...
def main():
    print("=" * 132)
    print("WIDE CENSUS — DISK-BOUNDED RESUMABLE FROZEN DETECTOR EXECUTION v056 (DASCH SIGNED-URL FRESHNESS FIX 4)")
    print("=" * 132)
