...
FILES_BASE = "https://www.plate-archive.org/files/"
...
def polygon_center_icrs(poly):
    """Deterministic spherical center of a frozen v052 sky polygon."""
    vectors = np.asarray([unitvec(ra, dec) for ra, dec in poly], dtype=float)
    v = np.sum(vectors, axis=0)
    n = float(np.linalg.norm(v))
    if not math.isfinite(n) or n <= 1e-12:
        raise RuntimeError("Degenerate frozen footprint polygon center")
    v /= n
    ra = math.degrees(math.atan2(float(v[1]), float(v[0]))) % 360.0
...
def resolve_datalink(archive_id, plate_id, filename):
    # APPLAUSE DR4 scan.filename_scan is already the repository-relative
    # path, e.g. DR4/scans/HAM-LA/LA07334_y.fits. The official Datalink
    # service exposes #this URLs under /files/<filename_scan>. Parsing the
    # human HTML Datalink page is therefore unnecessary and proved brittle.
    rel = str(filename).strip().replace("\\", "/").lstrip("/")
    if not rel.lower().startswith("dr4/scans/") or not rel.lower().endswith(".fits"):
        raise RuntimeError(
            f"APPLAUSE {archive_id}:{plate_id}: unexpected filename_scan path {filename!r}"
        )

    fits_url = urllib.parse.urljoin(FILES_BASE, rel)
    ...
    try:
        ...
        # HEAD support is not required. A bounded Range probe below is the
        # authoritative existence/type check and reads only the first FITS
        # header block, never science pixels.
        headers = {}
    ...
    with urllib.request.urlopen(range_req, timeout=120) as response:
        probe = response.read(2880)
        range_status = getattr(response, "status", None)
        range_final = response.geturl()
        range_headers = {k.lower(): v for k, v in response.headers.items()}

    if len(probe) < 80 or not probe.startswith(b"SIMPLE"):
        raise RuntimeError(
            f"APPLAUSE {archive_id}:{plate_id}: direct DR4 scan URL did not "
            f"return a FITS primary header block: {fits_url}"
        )

    return {
        "datalink_resolution_method": "DIRECT_FROM_DR4_SCAN_FILENAME_PLUS_FITS_HEADER_RANGE_PROBE",
        "repository_relative_filename_scan": rel,
        "fits_url": fits_url,
        "head_http_status": head_status,
        "head_final_url": head_final,
        "head_content_length": inum(headers.get("content-length")),
        "head_content_type": str(headers.get("content-type", "")),
        ...
        "range_probe_http_status": range_status,
        "range_probe_final_url": range_final,
        "range_probe_content_type": str(range_headers.get("content-type", "")),
        "range_probe_content_range": str(range_headers.get("content-range", "")),
        "fits_primary_header_block_sha256": hashlib.sha256(probe).hexdigest(),
        "fits_primary_header_magic_pass": True,
        "science_pixel_bytes_read": 0,
    }


def main():
    print("=" * 132)
    print("WIDE CENSUS — HEAVY DETECTOR RUN METADATA/CAPACITY PREFLIGHT v054 (FILE RESOLUTION FIX 3)")
    print("=" * 132)
    ...
    for q in qrows:
        ...
        for side in ("a", "b"):
            ...
            if kind == "APPLAUSE":
                # v052 candidates intentionally freeze the exact polygon + IDs,
                # not redundant solution_ra/dec fields. Derive the WCS-selection
                # target deterministically from that already-frozen polygon.
                center_ra, center_dec = polygon_center_icrs(cand["polygon"])
                endpoint_key = str(exposure).strip()
                ...
                record = {
                    ...
                    "solution_ra_deg": center_ra,
                    "solution_dec_deg": center_dec,
                    "solution_center_source": "SPHERICAL_CENTER_OF_FROZEN_V052_STC_POLYGON",
                ...
                record = {
                    ...
                    "solution_ra_deg": center_ra,
                    "solution_dec_deg": center_dec,
                    "solution_center_source": "SPHERICAL_CENTER_OF_FROZEN_V052_TPV_POLYGON",
                    "footprint_polygon": cand["polygon"],
                    "solution_center_sep_deg": float(
                        cand["center_sep_from_exposure_deg"]
                    ),
    ...
    wcs_cache = CACHE / "applause_selected_solution_rows.json"
    ...
    if not wcs_cache.is_file():
        key = "tap:selected_solution"
        if key in cp["terminal"]:
            raise RuntimeError("APPLAUSE selected-solution TAP query reached terminal failure")
        try:
            solution_ids = sorted({
                int(ep["solution_id"])
                for ep in applause_eps
            })
            ids = ",".join(str(x) for x in solution_ids)
            rows, meta = tap_query(
                "SELECT solution_id,solutionset_id,process_id,scan_id,plate_id,"
                "archive_id,solution_num,ra_icrs,dec_icrs,header_wcs "
                f"FROM applause_dr4.solution WHERE solution_id IN ({ids}) "
                "ORDER BY solution_id"
            )
            write_json(wcs_cache, {"meta": meta, "rows": rows})
            print(
                f"APPLAUSE exact selected-solution WCS metadata: "
                f"{len(rows)} rows for {len(solution_ids)} solution IDs"
            )
        except Exception as exc:
            n = mark_failure(cp, key, exc)
            print(
                f"APPLAUSE selected-solution TAP retry "
                f"{n}/{MAX_TRANSPORT_ATTEMPTS}: "
                f"{type(exc).__name__}: {exc}"
            )
            return 10
    ...
    wcs_by_solution = {}
    for row in wcs_rows:
        solution_id = inum(row.get("solution_id"))
        if solution_id is not None:
            wcs_by_solution.setdefault(solution_id, []).append(row)
    ...
    for ep in applause_eps:
        ...
        selected_rows = wcs_by_solution.get(int(ep["solution_id"]), [])
        if len(selected_rows) != 1:
            raise RuntimeError(
                f"APPLAUSE endpoint {ep['endpoint_key']}: expected exactly one "
                f"DR4 solution row for frozen solution_id={ep['solution_id']}, "
                f"got {len(selected_rows)}"
            )

        selected = selected_rows[0]

        # Cross-check that the direct row still describes the same frozen
        # scan/process/solution identity selected by v052.
        expected_identity = (
            int(ep["scan_id"]),
            int(ep["process_id"]),
            int(ep["solution_num"]),
        ...
        actual_identity = (
            inum(selected.get("scan_id")),
            inum(selected.get("process_id")),
            inum(selected.get("solution_num")),
            inum(selected.get("plate_id")),
            inum(selected.get("archive_id")),
        )
        if actual_identity != expected_identity:
            raise RuntimeError(
                f"APPLAUSE endpoint {ep['endpoint_key']}: frozen solution identity "
                f"changed: expected={expected_identity} actual={actual_identity}"
            )

        header_text = str(selected.get("header_wcs") or "")
        if not header_text.strip():
            raise RuntimeError(
                f"APPLAUSE endpoint {ep['endpoint_key']}: exact selected "
                f"solution_id={ep['solution_id']} has no header_wcs"
            )

        # DR4 solution.header_wcs is the adopted WCS for this exact solution.
        # We still allow a FITS header containing alternate WCS keys, but select
        # the key whose image center is nearest the already-frozen v052 polygon.
        ...
        ep["solutionset_id"] = inum(selected.get("solutionset_id"))
        ep["direct_solution_ra_deg"] = fnum(selected.get("ra_icrs"))
        ep["direct_solution_dec_deg"] = fnum(selected.get("dec_icrs"))
        ep["wcs_source"] = "APPLAUSE_DR4_SOLUTION_HEADER_WCS_EXACT_SOLUTION_ID"
    ...
    for ep in pending_links[:DATALINK_BATCH]:
        ...
        try:
            ...
            # If the server reports the full size on HEAD, it must agree with
            # the frozen DR4 scan-table metadata before this endpoint is
            # detector-ready.
            reported = rec.get("head_content_length")
            expected_size = int(ep["file_size"])
            if reported is not None and int(reported) != expected_size:
                raise RuntimeError(
                    f"APPLAUSE {ep['endpoint_key']}: repository file size "
                    f"{reported} != DR4 scan.file_size {expected_size}"
                )
    ...
    efields = [
        ...
        "solution_id", "solutionset_id", "solution_num", "selected_expnum",
        "filename_scan", "naxis1", "naxis2", "file_size",
        "wcs_key", "wcs_source", "wcs_center_match_sep_deg",
        "direct_solution_ra_deg", "direct_solution_dec_deg",
        "fits_url", "datalink_resolution_method",
        "repository_relative_filename_scan",
        "head_http_status", "head_content_length", "head_accept_ranges",
        "range_probe_http_status", "range_probe_content_range",
        "fits_primary_header_magic_pass",
        "fits_checksum", "fits_datasum",
    ]
    ...
    payload = {
        ...
        "applause_file_resolution": (
            "Use applause_dr4.scan.filename_scan as the repository-relative DR4 "
            "scan path and resolve https://www.plate-archive.org/files/<filename_scan>. "
            "Validate with a bounded first-2880-byte FITS header Range probe; no "
            "science pixel bytes are read during preflight."
        ),
        "selection_policy": {
            ...
            "applause_wcs": (
                "Query applause_dr4.solution by the exact v052-frozen solution_id; "
                "cross-check scan/process/solution_num/plate/archive identity, then "
                "use that row's adopted header_wcs. If alternate WCS keys are present "
                "inside the FITS header, choose the key whose image center is nearest "
                "the already-frozen v052 polygon center."
            ),
            "dasch_wcs": (
