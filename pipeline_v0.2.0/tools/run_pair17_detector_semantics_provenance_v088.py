...
def detector_source_candidates():
    scored = []
    for p in ROOT.rglob("*.py"):
        try:
            ...
            continue
        score = 0
        for term in ("polarity", "global_x", "global_y", "local_x", "candidate_index"):
            if term in t:
                score += 1
        if score >= 4:
...
def main():
    print("="*120)
    print("PAIR 17 — DETECTOR SEMANTICS / PROVENANCE AUDIT v088")
    print("="*120)
    ...
    # Locate frozen detector implementation.
    candidates = detector_source_candidates()
    if not candidates:
        raise RuntimeError("Could not locate detector implementation source")
    best_score = candidates[0][0]
    tied = [x for x in candidates if x[0] == best_score]
    # Prefer an exact detector.py among equally good candidates.
    exact = [x for x in tied if x[1].name.lower() == "detector.py"]
    chosen = exact[0] if len(exact) == 1 else (tied[0] if len(tied) == 1 else None)
    if chosen is None:
        raise RuntimeError(
            "Ambiguous detector source candidates: "
            + "; ".join(str(x[1].relative_to(ROOT)) for x in tied[:10])
        )
    _, det_path, det_text = chosen
