    ...
    if corrected_margin>OLD_MARGIN_ARCSEC:
        for ira,idec in existing_required:
            for leaf in complete_leaves(ira,idec):
                ...
                # Center must be identical. Compare the stored/reconstructed
                # coordinate components directly rather than using acos-based
                # spherical separation: acos(dot) can report a tiny non-zero
                # separation for numerically identical unit vectors.
                dra=abs(((nra-leaf["old_ra"]+180.0)%360.0)-180.0)
                ddec=abs(ndec-leaf["old_dec"])
                if dra>1e-8 or ddec>1e-8:
                    raise RuntimeError(
                        f"REFUSING: leaf center changed for {leaf['key']}: "
                        f"dRA={dra:.12g} deg dDec={ddec:.12g} deg"
                    )
