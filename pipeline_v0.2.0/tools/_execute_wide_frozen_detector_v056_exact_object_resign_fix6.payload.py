...
DASCH_MOSAIC_TRANSPORT_API = "https://api.starglass.cfa.harvard.edu/public/plates/p/{plate}/mosaic?bin_factor=01"
DASCH_UA = "historical-transient-pipeline/0.2.0-wide-census-v056"
DASCH_TRANSPORT_REFRESH_ATTEMPTS = 6
DASCH_MIN_SIGNED_URL_VALIDITY_S = 45
DASCH_MAX_INLINE_TRANSPORT_REFRESHES = 6
...
def dasch_geometry_sig(pkg):
    """Scientific geometry identity; excludes ephemeral signed transport URL."""
    a = pkg["metadata"]["astrometry"]
    m = pkg["metadata"]["mosaic"]
    obj = {
        "b01HeaderGz": a["b01HeaderGz"],
        "rotationDelta": a.get("rotationDelta"),
        "b01Height": int(m["b01Height"]),
        "b01Width": int(m["b01Width"]),
    }
...
def signed_url_remaining_seconds(url):
    q = urllib.parse.parse_qs(urllib.parse.urlsplit(url).query)
    issued = (q.get("X-Amz-Date") or q.get("x-amz-date") or [None])[0]
    expires = (q.get("X-Amz-Expires") or q.get("x-amz-expires") or [None])[0]
    if not issued or expires is None:
        return None
    try:
        t0 = datetime.strptime(str(issued), "%Y%m%dT%H%M%SZ").replace(tzinfo=timezone.utc)
        return (t0.timestamp() + float(expires)) - datetime.now(timezone.utc).timestamp()
    except Exception:
        return None


def stable_object_url(url):
...
def is_dasch_transport_exception(exc):
    s = f"{type(exc).__name__}: {exc}".lower()
    if "refusing:" in s:
        return False
    markers = (
        "403", "forbidden", "request has expired", "requestexpired",
        "expiredtoken", "signaturedoesnotmatch", "file not found",
        "filenotfounderror", "dasch-prod-user.s3",
        "transport credential refresh failed",
        "mosaic transport returned near-expiry credential",
        "presigned_link",
    )
    return any(m in s for m in markers)


def close_endpoint_handle(handle):
    if handle is None:
        return
    try:
        handle["hdul"].close()
        ...
        pass
    try:
        if handle.get("fileobj") is not None:
            handle["fileobj"].close()
    except Exception:
        pass


def refresh_dasch_package(plate, frozen_pkg):
    """
    Refresh only the ephemeral credential for the exact frozen DR7 base mosaic.

    v052's mosaic_package metadata remains the sole science-geometry authority.
    The public Starglass GET /plates/p/{plate}/mosaic endpoint is used only to
    obtain a new 15-minute presigned URL for bin_factor=01.

    Exact-object guards:
      * returned plate_id must equal the frozen endpoint plate;
      * returned bin_factor must be 01;
      * object_size must equal frozen baseFitsSize;
      * query-stripped object URL must exactly equal the object path frozen in
        v052's baseFitsUrl.

    Therefore this function cannot silently switch mosaics or WCS solutions.
    """
    plate = str(plate).strip().lower()
    frozen_url = str(frozen_pkg.get("baseFitsUrl") or "")
    frozen_size = int(frozen_pkg.get("baseFitsSize") or 0)
    if not frozen_url:
        raise RuntimeError(
            "REFUSING: frozen v052 DASCH package lacks baseFitsUrl object identity"
        )
    if frozen_size < 10_000_000:
        raise RuntimeError(
            f"REFUSING: frozen v052 DASCH baseFitsSize implausible: {frozen_size}"
        )

    frozen_stable = stable_object_url(frozen_url)
    frozen_path = urllib.parse.urlsplit(frozen_stable).path
    if plate not in frozen_path.lower():
        raise RuntimeError(
            f"REFUSING: frozen v052 DASCH object path does not identify plate "
            f"{plate}: {frozen_path}"
        )

    last_exc = None
    api_url = DASCH_MOSAIC_TRANSPORT_API.format(
        plate=urllib.parse.quote(plate, safe="")
    )

    for attempt in range(1, DASCH_TRANSPORT_REFRESH_ATTEMPTS + 1):
        try:
            ...
            returned_plate = str(fresh.get("plate_id") or "").strip().lower()
            if returned_plate != plate:
                raise RuntimeError(
                    f"REFUSING: transport endpoint returned plate_id "
                    f"{returned_plate!r}, expected {plate!r}"
                )

            bin_factor = str(fresh.get("bin_factor") or "").strip()
            if bin_factor.isdigit():
                bin_factor = f"{int(bin_factor):02d}"
            if bin_factor != "01":
                raise RuntimeError(
                    f"REFUSING: transport endpoint returned bin_factor "
                    f"{bin_factor!r}, expected '01'"
                )

            url = str(fresh.get("presigned_link") or "")
            size = int(fresh.get("object_size") or 0)
            if not url:
                raise RuntimeError(
                    "DASCH mosaic transport endpoint returned no presigned_link"
                )
            if size != frozen_size:
                raise RuntimeError(
                    f"REFUSING: transport object_size {size} != frozen "
                    f"baseFitsSize {frozen_size}"
                )

            stable = stable_object_url(url)
            if stable != frozen_stable:
                raise RuntimeError(
                    "REFUSING: transport endpoint presigned a different DASCH "
                    f"object than v052 froze: fresh={stable!r} "
                    f"frozen={frozen_stable!r}"
                )

            remaining_s = signed_url_remaining_seconds(url)
            if remaining_s is None:
                raise RuntimeError(
                    "DASCH mosaic transport returned signed URL without "
                    "parseable expiry"
                )
            if remaining_s < DASCH_MIN_SIGNED_URL_VALIDITY_S:
                last_exc = RuntimeError(
                    f"DASCH mosaic transport returned near-expiry credential: "
                    f"remaining={remaining_s:.1f}s"
                )
                if attempt < DASCH_TRANSPORT_REFRESH_ATTEMPTS:
                    time.sleep(min(8.0, 1.5 * attempt))
                    continue
                raise last_exc

            return {
                # Compatibility: science geometry remains the frozen package.
                "package": frozen_pkg,
                "signed_url": url,
                "stable_object_url": stable,
                "package_response_sha256": None,
                "transport_response_sha256": hashlib.sha256(raw).hexdigest(),
                "transport_api": api_url,
                "transport_kind": "STARGASS_PUBLIC_GET_MOSAIC_EXACT_FROZEN_OBJECT",
                "geometry_sha256": dasch_geometry_sig(frozen_pkg),
                "baseFitsSize": size,
                "refresh_attempt": attempt,
                "refreshed_at_utc": datetime.now(timezone.utc).isoformat(),
                "signed_url_remaining_seconds_at_refresh": remaining_s,
                "returned_mosaic_num": fresh.get("mosaic_num"),
                "returned_rotation": fresh.get("rotation"),
                "returned_flags": fresh.get("flags"),
            }

        except RuntimeError as exc:
            # Exact-object / provenance guards are not transport failures and
            # must never be retried or bypassed.
    ...
    raise RuntimeError(
        f"DASCH transport credential refresh failed after "
        f"{DASCH_TRANSPORT_REFRESH_ATTEMPTS} attempts for {plate}: "
...
def open_endpoint(endpoint):
    ...
    if kind == "APPLAUSE":
        ...
        return {
            "kind": kind,
            "hdul": hdul,
            "audit_source_url": endpoint["fits_url"],
            "transport_method": "APPLAUSE_DIRECT_STABLE_DR4_FILE_URL",
            "hdu": hdu,
            "hdu_index": hi,
            "wcs": endpoint["_wcs"],
            "rotation_k": 0,
    ...
    if kind == "DASCH":
        frozen_pkg = endpoint["_pkg"]
        plate = str(endpoint["plate_id"]).strip().lower()

        # v052's baseFitsUrl query string is an expired 15-minute credential,
        # but its query-stripped object path remains exact frozen object identity.
        # Re-sign that exact object through Starglass's independent public
        # GET-mosaic transport endpoint. Frozen v052 geometry remains authoritative.
        transport = refresh_dasch_package(plate, frozen_pkg)
        url = transport["signed_url"]

        # Avoid fsspec.open()'s path/glob convenience layer: a pre-signed URL
        # necessarily contains '?' and can cause an underlying HTTP failure to
        # be masked as a "glob characters" FileNotFoundError. Opening directly
        # through HTTPFileSystem treats the query string as literal transport.
        ...
        # Frozen v052 package remains authoritative for science geometry.
        m = frozen_pkg["metadata"]["mosaic"]
        ...
        return {
            ...
            "hdu_index": hi,
            "wcs": endpoint["_wcs"],
            "rotation_k": rotation_k(frozen_pkg),
            "base_shape": base_shape,
            "audit_source_url": transport["stable_object_url"],
            "transport_method": "FRESH_STARGASS_GET_MOSAIC_PRESIGNED_URL_EXACT_FROZEN_OBJECT",
            "transport_refresh": {
                "transport_response_sha256": transport["transport_response_sha256"],
                "transport_api": transport["transport_api"],
                "transport_kind": transport["transport_kind"],
                "geometry_sha256": transport["geometry_sha256"],
                "baseFitsSize": transport["baseFitsSize"],
                "refresh_attempt": transport["refresh_attempt"],
                "refreshed_at_utc": transport["refreshed_at_utc"],
                "signed_url_remaining_seconds_at_refresh": transport["signed_url_remaining_seconds_at_refresh"],
                "returned_mosaic_num": transport["returned_mosaic_num"],
                "returned_rotation": transport["returned_rotation"],
                "returned_flags": transport["returned_flags"],
                "exact_query_stripped_object_identity_match": True,
            },
        }
...
def run_one_tile(tile, endpoint, handle, method, contract_sha):
    ...
    try:
        ...
        # frozen detector.detect_array() raises ValueError when robust sigma
        # is non-finite or <= 0. An exact sigma==0 result is deterministic
        # for the same native pixels and means the detector has no defined
    ...
    meta = {
        ...
        "exposure": endpoint["exposure"],
        "tile_id": tile["tile_id"],
        "source_url": handle.get("audit_source_url", endpoint["fits_url"]),
        "source_transport_method": handle.get("transport_method"),
        "source_transport_refresh": handle.get("transport_refresh"),
        "signed_transport_query_persisted": False,
...
def main():
    print("=" * 132)
    print("WIDE CENSUS — DISK-BOUNDED RESUMABLE FROZEN DETECTOR EXECUTION v056 (DASCH EXACT-OBJECT RESIGN FIX 6)")
    print("=" * 132)
    ...
    for endpoint_key in order:
        ...
        try:
            ...
            for tile in groups[endpoint_key]:
                ...
                while True:
                    try:
                        ...
                        if (
                            ep["kind"] == "DASCH"
                            and is_dasch_transport_exception(exc)
                            and transport_refreshes < DASCH_MAX_INLINE_TRANSPORT_REFRESHES
                        ):
                            transport_refreshes += 1
                            print(
                                f"  {tkey} DASCH transport expired/failed; "
                                f"refreshing endpoint credential "
                                f"{transport_refreshes}/{DASCH_MAX_INLINE_TRANSPORT_REFRESHES}",
                                flush=True,
                            )
                            close_endpoint_handle(handle)
                            handle = None
                            try:
                                handle = open_endpoint(ep)
                                continue
                                ...
                                exc = refresh_exc
                                if (
                                    is_dasch_transport_exception(exc)
                                    and transport_refreshes < DASCH_MAX_INLINE_TRANSPORT_REFRESHES
                                ):
                                    time.sleep(min(8, 2 * transport_refreshes))
                                    continue

                        # Only a non-transport failure, or exhausted inline
                        # transport recovery, consumes the persistent tile
                        # process-attempt budget.
            ...
            # If the endpoint cannot be opened even after the transport-only
            # exact-object credential refresh has exhausted its attempts, retain this as
            # an explicit operational hold. It is never a scientific negative.
            for tile in groups[endpoint_key]:
                tkey = f"{endpoint_key}|{tile['tile_id']}"
                if checkpoint_valid(tile, contract_sha):
                    continue
