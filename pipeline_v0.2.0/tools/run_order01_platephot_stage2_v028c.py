...
WORK = ROOT / "work" / "order01_platephot_stage2_v028c"
...
CUM_MANIFEST = BASE / "order01_platephot_stage2_cumulative_manifest_v028c.csv"
NEW_MANIFEST = BASE / "order01_platephot_stage2_new_call_manifest_v028c.csv"
POLICY = BASE / "order01_platephot_stage2_policy_v028c.json"
DETAIL_NEW = BASE / "order01_platephot_stage2_detail_new_v028c.csv"
PLATES_NEW = BASE / "order01_platephot_stage2_plate_summary_new_v028c.csv"
RANK_CUM = BASE / "order01_platephot_stage2_rank_summary_cumulative_v028c.csv"
REPORT = BASE / "order01_platephot_stage2_report_v028c.json"

for d in (WORK, CACHE):
    d.mkdir(parents=True, exist_ok=True)

API = "https://api.starglass.cfa.harvard.edu/public/dasch/dr7/platephot"
UA = "historical-transient-pipeline/0.2.8-order01-platephot-stage2-v028c"
...
EXPECTED_STAGE1_CLEAN_RANKS = [5, 10, 12, 24, 25, 26, 29, 30, 36]
EXPECTED_STAGE1_RECURRENT_RANKS = [6, 8]
ACTIVE_RANKS = list(EXPECTED_STAGE1_CLEAN_RANKS)
RECURRENT_STAGE1_RANKS = list(EXPECTED_STAGE1_RECURRENT_RANKS)
...
def recurrent_rank_disposition(rank, stage1_detail, stage1_report):
    s = next(
        r for r in stage1_report["rank_summaries"]
        if int(r["strict_rank"]) == int(rank)
    )
    ...
    return {
        ...
        "reason": (
            "Stage 1 met the prospectively fixed >=2 independent physical-plate "
            "recurrence criterion within the 5-arcsec diagnostic gate. The rank "
            "is retained for audit but excluded from deeper recurrence expansion."
        ),
    }


def main():
    global ACTIVE_RANKS, RECURRENT_STAGE1_RANKS

    print("=" * 104)
    print("ORDER 01 — DASCH PLATEPHOT RECURRENCE STAGE 2: SAME BLIND PREFIX TO 256 v028c")
    print("=" * 104)
    print(
        "Stage-1-clean ranks only. Reproduce the exact first-64 prefix, then add "
        "blind prefix positions 65..256. Stage-1 recurrent ranks remain audit-only."
    )
    print()
    ...
    clean = [int(x) for x in stage1_report.get("stage1_clean_ranks_5arcsec", [])]
    recurrent = [int(x) for x in stage1_report.get("stage1_recurrent_ranks_5arcsec", [])]
    input_ranks = [int(x) for x in stage1_report.get("catalog_recurrence_clean_active_ranks", [])]

    ACTIVE_RANKS = clean
    RECURRENT_STAGE1_RANKS = recurrent
    ...
    guards = {
        ...
        "stage1_input_ranks": input_ranks == EXPECTED_STAGE1_INPUT_RANKS,
        "stage1_clean_ranks": clean == EXPECTED_STAGE1_CLEAN_RANKS,
        "stage1_recurrent_ranks": recurrent == EXPECTED_STAGE1_RECURRENT_RANKS,
        "stage1_selected_calls": int(stage1_report.get("selected_call_count", -1)) == 704,
        "stage1_completed_calls": int(stage1_report.get("completed_call_count", -1)) == 704,
        "stage1_failed_calls": int(stage1_report.get("failed_call_count", -1)) == 0,
        "stage1_report_sha": sha256_file(STAGE1_REPORT) == STAGE1_REPORT_SHA,
        ...
        "stage1_detail_sha": sha256_file(STAGE1_DETAIL) == STAGE1_DETAIL_SHA,
        "stage1_plates_sha": sha256_file(STAGE1_PLATES) == STAGE1_PLATES_SHA,
        "exposure_census_sha": sha256_file(EXPOSURE_CENSUS) == EXPOSURE_CENSUS_SHA,
    ...
    for rank in RECURRENT_STAGE1_RANKS:
        if not s1_by_rank[rank]["multi_independent_plate_recurrence_5arcsec"]:
            raise RuntimeError(
                f"REFUSING: rank {rank} is listed recurrent but did not meet the Stage-1 gate"
            )
    ...
    recurrent_audit = [
        recurrent_rank_disposition(rank, stage1_detail, stage1_report)
        for rank in RECURRENT_STAGE1_RANKS
    ]
    ...
    policy_obj = {
        "analysis_kind": "order01_platephot_recurrence_stage2_fixed_v028c",
        "active_ranks": ACTIVE_RANKS,
        "stage1_recurrent_ranks_audit_only": RECURRENT_STAGE1_RANKS,
        ...
        "minimum_independent_recurrence_plates": MIN_INDEPENDENT_RECURRENCE_PLATES,
        "stage1_recurrent_ranks_excluded_from_expansion_because_gate_met": True,
    ...
    print(f"Stage-1 recurrent/audit-only ranks: {RECURRENT_STAGE1_RANKS}")
    print(f"Stage-2 active ranks:               {ACTIVE_RANKS}")
    print("Exact Stage-1 first-64 prefix reproduction: PASS")
    ...
    for i, item in enumerate(new_calls, 1):
        ...
        try:
            ...
            print(
                f"  [{i:04d}/{len(new_calls):04d}] rank #{rank:02d} "
    ...
    for rank in ACTIVE_RANKS:
        ...
        if len(pids) != len(set(pids)):
            raise RuntimeError(
                f"REFUSING: cumulative rank {rank} contains duplicate physical plates"
            )
        ...
        rank_rows.append({
            ...
            "stage2_new_selected_plates": STAGE2_CUMULATIVE_PREFIX - STAGE1_PREFIX,
    ...
    report = {
        "status": status,
        "analysis_kind": "order01_platephot_recurrence_stage2_v028c",
        "guards": guards,
        "policy_sha256": policy_sha,
        "cumulative_manifest_sha256": cumulative_sha,
        "new_call_manifest_sha256": new_sha,
        "stage1_recurrent_rank_dispositions": recurrent_audit,
        "active_rank_summaries_cumulative_256": rank_rows,
        "stage2_recurrent_ranks_5arcsec": recurrent2,
        "stage2_clean_ranks_5arcsec": clean2,
        ...
        "stage1_recurrent_ranks_retained_in_audit": True,
        ...
        "next_stage_if_complete": (
            "For cumulative-256 ranks still without recurrence, expand the same "
            "blind ordering to cumulative 1024. Keep any Stage-2 recurrent ranks "
            "as static-contamination audit cases. Do not alter prior prefixes or "
            "the 3/5-arcsec gates."
        ),
    }
    ...
    print()
    print("=" * 104)
    print("ORDER 01 PLATEPHOT RECURRENCE STAGE 2 " + status)
    print("=" * 104)
    print(
    ...
    for a in recurrent_audit:
        print(
            f"strict #{int(a['strict_rank']):02d}: STAGE1_AUDIT_ONLY "
            f"<=5\" plates={a['plates_with_source_within_5arcsec']} "
    ...
    print()
    print("Stage-2 recurrent <=5\" ranks:", recurrent2)
    print("Stage-2 clean <=5\" ranks:    ", clean2)
    print()
    print("Outputs:")
    print(" ", REPORT)
    print(" ", POLICY)
