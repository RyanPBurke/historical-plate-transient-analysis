...
V082 = ROOT / "results" / "pair17_survivor_chronology_native_pixel_synthesis_v082"
V082_CLOSE = V082 / "pair17_close_time_native_pixel_measurements_v082.csv"
V082_BANK = V082 / "pair17_v082_bank_manifest.json"
...
EXPECTED = {
    ...
    V082_BANK:
        "fb0ab1a0d8d3afdd681bf24fd995086948f4b28fafa2473dd5752e067748338f",
    V075:
...
def measurement_map():
    """
    Return the exact banked native-pixel measurement used for each v083b panel.

    v080 contains the main comparison population. Two v082 close-time panels
    (293470 and 294052 on plate 7687 / scan 10000) were not present in v080;
    they were executed later in v082 with the exact frozen v080 method.
    Prefer v080 where a key already exists and use v082 only to fill missing keys.
    """
    out = {}
    ...
    for r in rows(V080_PLATES):
        ...
        out[key] = {**r, "_measurement_source": "BANKED_V080"}

    if not V082_CLOSE.is_file():
        raise RuntimeError(f"Missing banked v082 close-time measurements: {V082_CLOSE}")

    for r in rows(V082_CLOSE):
        key = (
            str(r.get("raw_match_row") or ""),
            integer(r.get("physical_plate_id")),
            integer(r.get("scan_id")),
        ...
        if key not in out:
            out[key] = {**r, "_measurement_source": "BANKED_V082_EXACT_FROZEN_V080_METHOD"}
...
def main():
    print("="*120)
    print("PAIR 17 — ASTROMETRIC LOCATOR AUDIT v086a")
    print("="*120)
    print("Post-unblind diagnostic audit")
    print("Operational repair:          merge banked v082 close-time measurements where v080 keys are absent")
    ...
    for p in panels:
        ...
        rec = {
            ...
            "measurement_source": (
                "SCIENCE_V075_SKY_COORDINATE"
                if science else m.get("_measurement_source","")
            ),
    ...
    report = {
        ...
        "operational_repair":"v086a merges banked v082 close-time native-pixel measurements only where the v080 key is absent; v082 used the exact frozen v080 method",
        "parent_v086_runner_sha256":"afc4defe3eebcbd6d6626852b7a54136cf80f4ca6455a7f95560f7d77aacecd8",
        "scientific_contract_changed_by_v086a":False,
    ...
    print()
    print("="*120)
    print("v086a LOCATOR AUDIT COMPLETE")
    print("="*120)
