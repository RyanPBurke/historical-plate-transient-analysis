...
def tree_nearest_dist(tree,gx,gy):
    if tree is None:return None
    d,_=tree.query([gx,gy],k=1)
    return float(d)


def build_native_index(rows,polarity=None):
    kept=[]; pts=[]
    for r in rows:
        ...
        kept.append(r);pts.append((rx,ry))
    if not pts:
        return {"tree":None,"rows":[]}
    return {"tree":cKDTree(np.asarray(pts,float)),"rows":kept}

def nearest_native_indexed(gx,gy,index,exclude_idx=None,maxdist=None):
    tree=index["tree"]
    if tree is None:return None
    k=min(8,len(index["rows"]))
    ds,inds=tree.query([gx,gy],k=k)
    ds=np.atleast_1d(ds);inds=np.atleast_1d(inds)
    for d,j in zip(ds,inds):
        r=index["rows"][int(j)]
        if exclude_idx is not None and i(r.get("candidate_index"))==exclude_idx:
            continue
        d=float(d)
        if maxdist is not None and d>maxdist:return None
        return d,r
    return None
...
def main():
    print("="*128)
    print("ORDER 01 — FOCUSED FINAL POSS MECHANISM AUDIT FOR ACTIVE #24/#30 v028ac-r1")
    print("="*128)
    ...
    positive_index_by_tile={
        tid:build_native_index(rows,polarity=1)
        for tid,rows in native_by_tile.items()
    }
    ...
    all_native_pts=np.asarray(
        [(f(r.get("global_x")),f(r.get("global_y"))) for r in native
         if f(r.get("global_x")) is not None and f(r.get("global_y")) is not None],float
    )
    all_gaia_pts=np.asarray([(r["global_x"],r["global_y"]) for r in gaia_pred],float)
    all_native_tree=cKDTree(all_native_pts) if len(all_native_pts) else None
    all_gaia_tree=cKDTree(all_gaia_pts) if len(all_gaia_pts) else None
    ...
    for gx in xs:
        for gy in ys:
            ...
            nd=tree_nearest_dist(all_native_tree,gx,gy)
            if nd is not None and nd<EDGE_EXCLUDE_NATIVE_PX:
                continue
            gd=tree_nearest_dist(all_gaia_tree,gx,gy)
            if gd is not None and gd<EDGE_EXCLUDE_GAIA_PX:
                continue
    ...
    pos_index=build_native_index(pos_native,polarity=1)
    ...
    for n in neg_native:
        ...
        q=nearest_native_indexed(gx,gy,pos_index)
    ...
    for g in bright_candidates.values():
        ...
        qp=nearest_native_indexed(
            gx,gy,positive_index_by_tile.get(tid,{"tree":None,"rows":[]}),
            maxdist=BRIGHT_POSITIVE_NATIVE_SEARCH_PX
        )
        neg=[]
        for n in nrows:
    ...
    payload={
        "stage":"ORDER01_ACTIVE24_30_FOCUSED_POSS_MECHANISM_V028AC_R1",
