...
def hpm_adql():
    return (
        f"SELECT TOP {MAX_HPM_ROWS} {gaia_select()} "
        "FROM gaiadr3.gaia_source "
        f"WHERE pm > {HPM_MASYR:.6f}"
    )
...
def main():
    ...
    print(
        f"Gaia J2016 search cone: {GAIA_CONE_ARCSEC:.0f}\"; "
        f"plus global pm>{HPM_MASYR:.0f} mas/yr safety query"
    )
    print()
    ...
    print("[1/3] Gaia high-proper-motion safety query ...", flush=True)
    hpm, hpm_meta, hpm_status = tap_query(hpm_adql(), "global_pm_gt_1500")
    if len(hpm) >= MAX_HPM_ROWS:
        raise RuntimeError(
            "REFUSING: global high-PM query hit TOP limit; completeness not assured"
        )
    print(f"  {hpm_status.upper()}: {len(hpm)} Gaia sources with pm>{HPM_MASYR:.0f} mas/yr")
    print()

    result_rows = []
    source_rows_all = []

    print("[2/3] Propagating Gaia candidates for all 23 strict associations ...", flush=True)
    ...
    for idx, r in enumerate(strict_raw, 1):
        ...
        cone, cone_meta, cone_status = tap_query(
            cone_adql(float(mid.ra.deg), float(mid.dec.deg)),
        ...
        for s in hpm:
            ...
            origins.setdefault(s["source_id"], set()).add("global_hpm")
        ...
        for sid, src in merged.items():
            ...
            # Keep all cone rows for audit, but global-HPM-only sources are
            # retained only when they approach the historical pair.
        ...
        print(
            f"  [{idx:02d}/23] strict #{rank:02d} {cone_status.upper():6s} "
            f"cone={len(cone):4d} best-both={neartext:>9s} "
            f"{q['gaia_class']}",
            flush=True,
        )
    ...
    # ------------------------------------------------------------
    print()
    print("[3/3] Normalizing DASCH tile concentration ...")
    ...
    report_out = {
        ...
        "predeclared_thresholds": {
            ...
            "global_hpm_threshold_masyr": HPM_MASYR,
        },
        "guards": guards,
        ...
        "global_hpm_source_count": len(hpm),
        "global_hpm_query_status": hpm_status,
