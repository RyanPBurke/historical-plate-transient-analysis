...
WORK = ROOT / "work" / "wide_census_gaia_registration_v068a"
OUT = RESULTS / "wide_census_gaia_registration_v068a"
...
def propagate_gaia(df, epoch_iso, chunk_size=250000):
    ...
    # v068a operational repair:
    # Parse offset-aware ISO-8601 through pandas, normalize to UTC,
    # then pass a timezone-naive UTC datetime explicitly to Astropy.
    # No scientific registration semantics are changed.
    ts = pd.Timestamp(epoch_iso)
...
def write_state(state):
    atomic_json(OUT / "state_v068a.json", state)
...
def main():
    ...
    print("=" * 100)
    print("WIDE CENSUS OFFLINE GAIA REGISTRATION v068a")
    print("=" * 100)
    ...
    state_path = OUT / "state_v068a.json"
    ...
    if args.resume and state_path.exists():
        ...
        state = {
            "analysis_kind":
                "wide_census_offline_gaia_registration_v068a",
            "status": "RUNNING",
    ...
    if state.get("script_sha256") != script_sha:
        raise RuntimeError(
            "Existing v068a state was created by a different script SHA."
        )
    ...
    for pair_index in wanted_pairs:
        pair_file = OUT / f"pair_{pair_index:02d}_registrations_v068a.csv"
        pair_json = OUT / f"pair_{pair_index:02d}_summary_v068a.json"
    ...
    if args.pair is None:
        ...
        if completed == list(range(1, EXPECTED_PAIRS + 1)):
            ...
            pd.DataFrame(summaries).to_csv(
                OUT / "wide_census_gaia_registration_pair_summary_v068a.csv",
                index=False,
            )

            print()
            print("=" * 100)
            print("v068a OFFLINE GAIA REGISTRATION COMPLETE")
            print("=" * 100)
            print(
