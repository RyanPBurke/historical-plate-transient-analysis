...
OUT_PAIR = BASE / "order01_dasch_catalog_recurrence_triage_v028c.csv"
OUT_SOURCES = BASE / "order01_dasch_catalog_recurrence_sources_v028c.csv"
OUT_LC = BASE / "order01_dasch_catalog_recurrence_lightcurves_v028c.csv"
OUT_REPORT = BASE / "order01_dasch_catalog_recurrence_report_v028c.json"
OUT_UNUSABLE = BASE / "order01_dasch_catalog_recurrence_unusable_querycat_rows_v028c.csv"
...
UA = "historical-transient-pipeline/0.2.8-order01-dasch-recurrence-v028c"
...
PAIR_FIELDS = [
    ...
    "atlas_query_rows",
    "apass_query_rows",
    "querycat_position_unusable_rows",
    "querycat_position_unusable_rows_num_matches_ge2",
    "querycat_position_unresolved_potential_recurrence",
...
def angular_sep_arcsec(ra1_deg, dec1_deg, ra2_deg, dec2_deg):
    """Stable great-circle separation in arcsec."""
...
def source_record(rank, refcat, row, query_ra_deg, query_dec_deg):
    """
    Resolve querycat source geometry conservatively.

    Preferred representation is the API-provided dra/ddec pair. DASCH also
    documents exported catalogue positions as ra_deg/dec_deg; if offsets are
    non-finite, reconstruct the exact great-circle separation from those
    coordinates and the already-fixed query centre.

    If neither representation is usable, return an audited unresolved record
    rather than silently declaring the candidate catalogue-clean.
    """
    raw_dra = getv(row, "dra")
    raw_ddec = getv(row, "ddec")
    dra = ffloat(raw_dra)
    ddec = ffloat(raw_ddec)

    src_ra = ffloat(getv(row, "ra_deg", "raDeg", "ra"))
    src_dec = ffloat(getv(row, "dec_deg", "decDeg", "dec"))
    ...
    if dra is not None and ddec is not None:
        ...
        sep = angular_sep_arcsec(
            query_ra_deg, query_dec_deg, src_ra, src_dec
        )

        # Reconstructed tangent-plane offsets are audit values only.
        dra_deg = ((src_ra - query_ra_deg + 180.0) % 360.0) - 180.0
        dra = (
            dra_deg
            * 3600.0
            * math.cos(math.radians(float(query_dec_deg)))
        )
        ddec = (src_dec - query_dec_deg) * 3600.0
        mode = "reconstructed_from_ra_dec"

        # querycat is a 60" box search. A wildly inconsistent reconstructed
        # coordinate means the row schema was misinterpreted; fail closed.
        if sep > QUERY_RADIUS_ARCSEC * math.sqrt(2.0) + 1.0:
            raise RuntimeError(
                f"{rank}/{refcat}: reconstructed querycat source separation "
                f"{sep:.3f}\" is inconsistent with the {QUERY_RADIUS_ARCSEC:.0f}\" "
                "box-search contract"
            )
        ...
        return None, {
            ...
            "raw_dra": None if raw_dra is None else str(raw_dra),
            "raw_ddec": None if raw_ddec is None else str(raw_ddec),
            "reason": "NO_FINITE_DRA_DDEC_OR_RA_DEC",
        }
...
def main():
    print("=" * 92)
    print("ORDER 01 — DASCH DR7 CATALOGUED HISTORICAL RECURRENCE SCREEN v028c")
    print("=" * 92)
    print(
    ...
    for idx, rank in enumerate(EXPECTED_SURVIVORS, 1):
        ...
        for refcat in REFCATS:
            ...
            parsed = []
            for row in qrows:
                src, unusable = source_record(
                    rank, refcat, row, ra, dec
                )
                if unusable is not None:
                    unusable_rows.append(unusable)
                    continue
                parsed.append(src)
        ...
        rank_unusable = [
            u for u in unusable_rows
            if int(u["strict_rank"]) == rank
        ]
        rank_unusable_ge2 = [
            u for u in rank_unusable
            if int(u["num_matches"] or 0) >= MIN_QUERYCAT_MATCHES_TO_FETCH_LC
        ]
        unresolved_potential = bool(rank_unusable_ge2)
        ...
        if strong:
            ...
            if unresolved_potential:
                cls = "DASCH_QUERYCAT_POSITION_UNRESOLVED_RETAIN_FOR_PLATEPHOT"
                best = close_unconfirmed[0] if close_unconfirmed else (
        ...
        pair_rows.append({
            ...
            "querycat_position_unusable_rows": len(rank_unusable),
            "querycat_position_unusable_rows_num_matches_ge2": len(rank_unusable_ge2),
            "querycat_position_unresolved_potential_recurrence": unresolved_potential,
    ...
    write_csv(OUT_UNUSABLE, unusable_rows, UNUSABLE_QUERYCAT_FIELDS)
    ...
    out_report = {
        "status": "COMPLETE",
        "analysis_kind": "order01_dasch_dr7_catalogued_historical_recurrence_v028c",
        "api_base": API_BASE,
        "guards": guards,
        "implementation_amendment": {
            "supersedes_for_downstream_use":
                "order01_dasch_catalog_recurrence_report_v028.json",
            "reason": (
                "v028 assumed every querycat row had finite dra/ddec. "
                "v028b retains the same recurrence policy but resolves a missing "
                "offset pair from documented ra_deg/dec_deg when available. "
                "Rows with neither usable representation are explicitly audited; "
                "if num_matches>=2 they conservatively keep the rank eligible for "
                "blind platephot rather than being silently ignored."
            ),
            "science_thresholds_changed": False,
            "v028c_helper_fix": (
                "Restored the boolean parser used only for the descriptive "
                "same_polarity output field; recurrence calculations are unchanged."
            ),
            "3arcsec_gate_unchanged": True,
            "5arcsec_gate_unchanged": True,
            "minimum_distinct_plate_recurrence_unchanged": True,
        },
        "fixed_policy": {
            "refcats": REFCATS,
        ...
        "querycat_position_unusable_row_count": len(unusable_rows),
        "querycat_position_unusable_num_matches_ge2_count": sum(
            int(u["num_matches"] or 0) >= MIN_QUERYCAT_MATCHES_TO_FETCH_LC
            for u in unusable_rows
        ),
        ...
        "outputs": {
            ...
            "unusable_querycat_rows_csv": str(OUT_UNUSABLE),
        },
        "next_stage": (
