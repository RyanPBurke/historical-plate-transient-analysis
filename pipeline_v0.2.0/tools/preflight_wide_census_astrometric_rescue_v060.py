...
def main():
    print("=" * 132)
    print("WIDE CENSUS — INDEPENDENT ASTROMETRIC RESCUE METADATA PREFLIGHT v060 (NONBLOCKING XMATCH FIX 1)")
    print("=" * 132)
...
"""
        ...
        print(
            f"             returned {len(srows)}; querying targeted xmatch diagnostics ...",
            flush=True,
        )

        # source_xmatch.process_id queries can trigger an archive-side sequential
        # scan and statement timeout. Xmatch evidence is explicitly diagnostic
        # only in the prospective v060 contract, so target a small deterministic
        # sample of already-returned source_ids instead. Failure remains a
        # diagnostic availability state and must not block centroid readiness.
        sample_ids = [
            inum(r.get("source_id"))
            for r in srows[:XMATCH_TOP]
            if inum(r.get("source_id")) is not None
        ]
        xrows = []
        xmeta_parts = []
        xmatch_error = None
        for batch_no, id_batch in enumerate(batched(sample_ids, XMATCH_ID_BATCH), 1):
            xq = f"""SELECT
  source_id,
  process_id,
  scan_id,
...
WHERE {or_filter("source_id", id_batch)}
ORDER BY source_id
"""
            try:
                part, xm = tap_query(
                    f"xmatch_process_{proc}_batch{batch_no}",
                    xq,
                )
                xrows.extend(part)
                xmeta_parts.append({
                    "batch_no": batch_no,
                    "source_id_count": len(id_batch),
                    "response": xm,
                })
            except Exception as exc:
                xmatch_error = f"{type(exc).__name__}: {exc}"
                print(
                    f"             xmatch diagnostic unavailable for process {proc}: "
                    f"{xmatch_error}",
                    flush=True,
                )
                break

        if xmatch_error is None:
            print(
                f"             xmatch diagnostic rows returned: {len(xrows)}",
                flush=True,
            )
            xmatch_query_meta[str(proc)] = {
                "status": "AVAILABLE",
                "query_mode": "targeted_source_id_batches",
                "requested_source_ids": len(sample_ids),
                "returned_rows": len(xrows),
                "batches": xmeta_parts,
            }
            for xr in xrows:
            ...
            xmatch_query_meta[str(proc)] = {
                "status": "XMATCH_DIAGNOSTIC_UNAVAILABLE",
                "query_mode": "targeted_source_id_batches",
                "requested_source_ids": len(sample_ids),
                "returned_rows_before_failure": len(xrows),
                "error": xmatch_error,
                "blocking": False,
            }
            for xr in xrows:
    ...
    for r in rescue:
        ...
        xmmeta = None if proc is None else xmatch_query_meta.get(str(proc), {})
        xmatch_status = (
            "NOT_APPLICABLE_NO_PROCESS"
            if proc is None
            else str((xmmeta or {}).get("status", "UNKNOWN"))
        )
        ...
        plate_out.append({
            ...
            **psummary,
            ...
            "existing_source_xmatch_diagnostic_status": xmatch_status,
    ...
    plate_fields = [
        ...
        "existing_source_xmatch_diagnostic_status",
    ...
    xmatch_status_counts = {}
    for meta in xmatch_query_meta.values():
        st = str(meta.get("status", "UNKNOWN"))
        xmatch_status_counts[st] = xmatch_status_counts.get(st, 0) + 1
    ...
    report = {
        ...
        "existing_xmatch_diagnostic": {
            ...
            "target_source_ids_per_process": XMATCH_TOP,
            "source_id_batch_size": XMATCH_ID_BATCH,
            "status_counts": xmatch_status_counts,
            "non_blocking_by_contract": True,
            "interpretation": (
