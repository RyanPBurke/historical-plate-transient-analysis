...
FREEZE=ROOT/"research/prospective_freezes/wide_census_registered_control_gaia_supplement_contract_v001.json"
V065=ROOT/"results/wide_census_registered_control_gaia_supplement_plan_v070/wide_census_registered_control_gaia_supplement_plan_v070.json"
PLAN=ROOT/"results/wide_census_registered_control_gaia_supplement_plan_v070/wide_census_registered_control_ordinary_query_plan_v070.csv"
HPLAN=ROOT/"results/wide_census_registered_control_gaia_supplement_plan_v070/wide_census_registered_control_hpm_query_plan_v070.csv"
V064=ROOT/"results/wide_census_gaia_supplemental_acquisition_v066/wide_census_gaia_supplemental_acquisition_v066.json"

OUTDIR=ROOT/"results/wide_census_registered_control_gaia_supplemental_acquisition_v071b"
CACHE=OUTDIR/"cache"
OCACHE=CACHE/"ordinary"
HCACHE=CACHE/"hpm"
STATE=OUTDIR/"state_v071b.json"
REPORT=OUTDIR/"wide_census_registered_control_gaia_supplemental_acquisition_v071b.json"
MANIFEST=OUTDIR/"wide_census_registered_control_gaia_supplemental_manifest_v071b.csv"

EXPECTED_FREEZE_SHA="91c7fb695ab2e664e93556bfba31acfbb81abf8665a5ac8ef8a7a64671073e6c"

EXPECTED_V070_REPORT_SHA="c614891c5bf8745ce6ff81d5a034a7e778fd002a3c3260ff75b74e3feafeeb2f"
EXPECTED_V070_ORDINARY_PLAN_SHA="0d5b41a329b92f36d4bfaa9c84d53454cea1f947448cb99ca69db18b4540b91d"
EXPECTED_V070_HPM_PLAN_SHA="0fa5f38fd2ac01b03a620d236c8593a29805b2b52aec9aa02c6a3edb22e8d473"
EXPECTED_PRIOR_V066_REPORT_SHA="b7d1c039b2cd317fc7077c0b6f715dd9fec31a5dd095b5c4d52d0a60b496f951"
EXPECTED_V070_PLANNER_SHA="ae76b497c7b27f35ed056748347ef7493605bc61b0d91531cf942ff1c3c28277"

V070_PLANNER=ROOT/"tools/plan_wide_census_registered_control_gaia_supplement_v070.py"

EXPECTED_SUPP=3009
EXPECTED_HPM=16
EXPECTED_NEW_FULL=3009
EXPECTED_ANNULUS=0

TAP="https://gea.esac.esa.int/tap-server/tap/sync"
UA="historical-transient-pipeline/wide-census-registered-control-gaia-supplemental-v071"
MAXREC=50000
...
def full_leaf(task,bounds,path,depth):
    ...
    m=save_complete("ordinary",key,adql,b,n,attempt,{
        "mode":"FULL_NEW_BASE_CELL_CONTROL",
...
def process_ordinary(task):
    ...
    if mode=="FULL_NEW_BASE_CELL_CONTROL":
        ira=int(task["base_cell_ira"]);idec=int(task["base_cell_idec"])
...
def process_hpm(r):
    ...
    log(f"[HPM pair {idx}/16] r={radius:.5f} deg pm>={pm:.0f}")
    b,n,attempt=curl_query(adql)
    if n>=MAXREC:
...
def main():
    ...
    if args.workers not in (1,2,3,4,5,6,7,8):
        raise RuntimeError("REFUSING: v071b supports only 1 to 8 workers; default remains 2 and operational maximum is 8")

    print("="*136)
    print("WIDE CENSUS — REGISTERED-CONTROL SUPPLEMENTAL GAIA DR3 ACQUISITION v071b")
    print("="*136)
    ...
    for p in (FREEZE,V065,PLAN,HPLAN,V064,V070_PLANNER):
        if not p.is_file():
            raise RuntimeError(f"REFUSING missing prerequisite {p}")

    if sha_file(FREEZE)!=EXPECTED_FREEZE_SHA:
        raise RuntimeError("REFUSING: v070 supplement contract SHA changed")

    frozen_inputs=(
        (V065,EXPECTED_V070_REPORT_SHA,"v070 report"),
        (PLAN,EXPECTED_V070_ORDINARY_PLAN_SHA,"v070 ordinary plan"),
        (HPLAN,EXPECTED_V070_HPM_PLAN_SHA,"v070 HPM plan"),
        (V064,EXPECTED_PRIOR_V066_REPORT_SHA,"prior v066 acquisition"),
        (V070_PLANNER,EXPECTED_V070_PLANNER_SHA,"v070 planner"),
    )

    for path,expected,label in frozen_inputs:
        actual=sha_file(path)
        if actual!=expected:
            raise RuntimeError(
                f"REFUSING: {label} SHA changed: "
                f"expected={expected} actual={actual}"
            )
    v65=json.loads(V065.read_text(encoding="utf-8"))
    if v65.get("status")!="COMPLETE":raise RuntimeError("REFUSING v070 plan incomplete")
    v64=json.loads(V064.read_text(encoding="utf-8"))
    if v64.get("status")!="COMPLETE":raise RuntimeError("REFUSING prior v066 acquisition incomplete")
    ...
    if modes.get("FULL_NEW_BASE_CELL_CONTROL",0)!=EXPECTED_NEW_FULL or modes.get("MARGIN_ANNULUS_EXISTING_LEAF",0)!=EXPECTED_ANNULUS:
    ...
    print("Scanning/verifying existing v071b cache once ...",flush=True)
    ...
    with ThreadPoolExecutor(max_workers=args.workers,thread_name_prefix="gaia-v071b") as ex:
    ...
    if args.max_new is not None and completed_this_run>=args.max_new and len(pending)>completed_this_run:
        ...
        with ThreadPoolExecutor(max_workers=args.workers,thread_name_prefix="gaia-v071ba-hpm") as ex:
    ...
    log("\nFINAL INTEGRITY SCAN: verifying v071b cache ...")
    manifest,done,leaves,rows,cb,hdone,hrows,hcb=scan_all(plan,hplan)
    ...
    rep={
        "status":status,
        "analysis_kind":"wide_census_registered_control_gaia_supplemental_acquisition_v071b",
        "updated_at_utc":datetime.now(timezone.utc).isoformat(),
        "guards":{
        ...
        "inputs":{
            "v070_supplement_contract_sha256":sha_file(FREEZE),
            "v070_plan_report_sha256":sha_file(V065),
            "v070_ordinary_plan_sha256":sha_file(PLAN),
            "v070_hpm_plan_sha256":sha_file(HPLAN),
            "prior_v066_acquisition_sha256":sha_file(V064),
        },
        "transport":{
            "workers":args.workers,
            ...
            "ordinary_annulus_maxrec_policy":"not_applicable_no_annulus_tasks",
        ...
        "registered_control_science_outcomes_inspected":0,
        "next_stage":"Run v072 registered-control coverage closure proof; only after zero ordinary/HPM gaps may registered controls execute.",
    }
    ...
    print(f"v071 compressed cache size:           {(cb+hcb)/(1024**3):.2f} GiB")
    ...
    print("REGISTERED-CONTROL SCIENCE OUTCOMES INSPECTED: 0")
