...
def patch_registry(text):
    if 'stage_id="dasch_platephot_full_queue_v028be"' in text:
    ...
    for node in tree.body:
        ...
        if isinstance(node, ast.Assign):
            ...
            for target in node.targets:
                if isinstance(target, ast.Name) and target.id == "ORDER01_STAGES":
                    target_name = target.id
                    break
        elif isinstance(node, ast.AnnAssign):
            value = node.value
            if isinstance(node.target, ast.Name) and node.target.id == "ORDER01_STAGES":
    ...
    if container is None:
        raise RuntimeError(
            "Could not locate ORDER01_STAGES list/tuple using Python AST."
        )

    lines = text.splitlines(keepends=True)
    # end_lineno points to the closing ] or ) line. Insert immediately before it.
    insert_index = container.end_lineno - 1
...
def patch_runner(text):
    ...
    # Generic version-banner normalization.
    text, n = re.subn(
        r'Transient automation v[0-9.]+ - Order01 registry status',
        'Transient automation v0.1.0 - Order01 registry status',
        text,
        count=1,
    )
    if n:
        notes.append("updated status banner to v0.1.0")

    # Add return-code-10 handling directly after the actual subprocess.run assignment,
    # locating it through the AST rather than source formatting.
    if "IN_PROGRESS_CHECKPOINTED" not in text:
        tree = ast.parse(text)
        fn = find_function(tree, "execute_stage")
        if fn is None:
            raise RuntimeError("execute_stage() not found in runner")
        ...
        for node in ast.walk(fn):
            if not isinstance(node, ast.Assign):
                continue
            if not any(
                isinstance(t, ast.Name) and t.id == "proc"
        ...
        if proc_assign is None:
            raise RuntimeError("subprocess.run assignment to proc not found in execute_stage")

        lines = text.splitlines(keepends=True)
        lines.insert(proc_assign.end_lineno, "\n" + PROGRESS_BLOCK)
        text = "".join(lines)
        ast.parse(text)
        notes.append(
            f"added return-code-10 progress handling after runner line {proc_assign.end_lineno}"
        )
    else:
        notes.append("return-code-10 progress handling already present")

    # Add looping command function before add_execution_flags().
    if "def cmd_run_until_blocked(" not in text:
        tree = ast.parse(text)
        marker_fn = find_function(tree, "add_execution_flags")
        if marker_fn is None:
            raise RuntimeError("add_execution_flags() not found in runner")

        lines = text.splitlines(keepends=True)
        lines.insert(marker_fn.lineno - 1, RUN_UNTIL_BLOCKED + "\n")
        text = "".join(lines)
        ast.parse(text)
        notes.append(
            f"added cmd_run_until_blocked() before runner line {marker_fn.lineno}"
        )
    else:
        notes.append("cmd_run_until_blocked() already present")

    # Register parser subcommand immediately before build_parser()'s `return p`.
    if 'sub.add_parser("run-until-blocked")' not in text:
        tree = ast.parse(text)
        fn = find_function(tree, "build_parser")
        if fn is None:
            raise RuntimeError("build_parser() not found in runner")

        return_p = None
        for node in fn.body:
            if isinstance(node, ast.Return) and isinstance(node.value, ast.Name) and node.value.id == "p":
                return_p = node
                break

        if return_p is None:
            raise RuntimeError("build_parser() return p statement not found")

        lines = text.splitlines(keepends=True)
        lines.insert(return_p.lineno - 1, PARSER_BLOCK)
        text = "".join(lines)
        ast.parse(text)
        notes.append(
            f"registered run-until-blocked parser before runner line {return_p.lineno}"
        )
    else:
        notes.append("run-until-blocked parser already present")
...
def main():
    print("=" * 112)
    print("TRANSIENT AUTOMATION REPAIR v0.1.0a — COMPLETE ABORTED v0.1.0 INSTALL")
    print("=" * 112)
    ...
    for p in (REGISTRY, RUNNER, STAGE, AUTO / "__init__.py"):
        if p.is_file():
            dst = BACKUP / p.name
            if not dst.exists():
                shutil.copy2(p, dst)

    # The failed v010 already created this file, but overwrite it with the exact
    # syntax-validated copy so the partially installed stage is deterministic.
    STAGE.parent.mkdir(parents=True, exist_ok=True)
    STAGE.write_text(STAGE_CONTENT, encoding="utf-8")
    py_compile.compile(str(STAGE), doraise=True)
    print(f"Stage file validated: {STAGE.relative_to(ROOT)}")

    reg = REGISTRY.read_text(encoding="utf-8")
    reg, reg_note = patch_registry(reg)
    REGISTRY.write_text(reg, encoding="utf-8")
    print(f"Registry: {reg_note}")

    runner = RUNNER.read_text(encoding="utf-8")
    runner, runner_notes = patch_runner(runner)
    RUNNER.write_text(runner, encoding="utf-8")
    for note in runner_notes:
        print(f"Runner: {note}")
    ...
    # Compile everything after repair.
    failures = []
    py_files = sorted(
    ...
    # Reparse the registry/runner as an extra structural guard.
    ast.parse(REGISTRY.read_text(encoding="utf-8"))
    ast.parse(RUNNER.read_text(encoding="utf-8"))

    print("\\nAUTOMATION REPAIR STATUS: PASS")
    print(f"Backup of pre-repair files: {BACKUP.relative_to(ROOT)}")
    print("\\nNext commands:")
    print(r'  & ".\\.venv\\Scripts\\python.exe" -m automation.runner status')
    print(r'  & ".\\.venv\\Scripts\\python.exe" -m automation.runner run-next')
    print("    expected: full queue stage selected, then REFUSED without --allow-network")
    print(r'  & ".\\.venv\\Scripts\\python.exe" -m automation.runner run-until-blocked --allow-network')
    print("\\nNo network request was made by this repair.")
    return 0
