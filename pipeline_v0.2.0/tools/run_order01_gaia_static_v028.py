...
OUT_REPORT = RESULT / "order01_gaia_static_report_v028.json"
OUT_TRIAGE = RESULT / "order01_gaia_static_triage_v028.csv"
OUT_SOURCES = RESULT / "order01_gaia_source_candidates_v028.csv"
    ...
    for attempt in range(1, attempts + 1):
        req = urllib.request.Request(
            ...
            headers={
                ...
                "Accept": "text/csv,*/*",
                "User-Agent": "historical-transient-pipeline/0.2.8-order01-gaia-static",
            },
        )
        try:
...
def main():
    print("=" * 112)
    print("ORDER 01 — GAIA DR3 STATIC-SOURCE EPOCH-PROPAGATION TRIAGE v028")
    print("=" * 112)
    print(
    ...
    normal_escape = ORDINARY_CONE_ARCSEC * 1000.0 / dt_gaia
    hpm_coverage = HPM_RESCUE_CONE_ARCSEC * 1000.0 / dt_gaia
    ...
    print(
        f'Ordinary 120" cone nominal PM escape scale: '
        f"{normal_escape:.1f} mas/yr"
    )
    print(f'900" rescue coverage scale: {hpm_coverage:.1f} mas/yr')
    print()
    ...
    for item in strict_rows:
        ...
        print(
            ...
            f'(120" ordinary + 900" high-PM rescue) ...',
            flush=True,
        )
        ...
        for row in hpm:
            ...
            if not sid:
                continue
            by_id[sid] = row
            origins.setdefault(sid, set()).add("hpm900_mu1800")
    ...
    report = {
        "status": "COMPLETE",
        "analysis_kind": "order01_gaia_dr3_static_epoch_propagation_v028",
        "guards": guards,
        ...
        "high_pm_strategy":
            "local_900arcsec_pm_gt_1800_rescue_per_strict_pair",
        ...
        "next_stage": (
            "Run Pan-STARRS DR2 repeated-static positional checks only on "
