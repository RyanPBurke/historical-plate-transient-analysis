...
OUT_ENDPOINT = OUTDIR / "order61_discovery_plate_endpoint_metrics.csv"
OUT_CONTROL = OUTDIR / "order61_discovery_plate_matched_controls.csv"
OUT_SUMMARY = OUTDIR / "order61_discovery_plate_candidate_summary.csv"
OUT_REPORT = OUTDIR / "order61_discovery_plate_audit_report.json"
...
def extract_display(arr, lx, ly, radius=DISPLAY_RADIUS_PX):
    ...
    if y0 < 0 or x0 < 0 or y1 > arr.shape[0] or x1 > arr.shape[1]:
        raise RuntimeError("candidate lacks full display cutout")
    return np.asarray(arr[y0:y1, x0:x1], dtype=float)
...
def main():
    print("=" * 108)
    print("ORDER 61 — DISCOVERY-PLATE NATIVE PIXEL + SNR-MATCHED PEER AUDIT v028")
    print("=" * 108)
    print(
    ...
    report = {
        "status": "COMPLETE",
        "analysis_kind": "order61_discovery_plate_native_pixel_snr_matched_peer_audit_v028",
        "guards": guards,
        "fixed_peer_policy": {
            "same_tile": True,
            "same_polarity": True,
