#!/usr/bin/env python3
"""
v077a — Operational repair of Pair 17 APPLAUSE independent-physical-plate opportunity census.
...
OUT = RESULTS / "pair17_applause_independent_plate_opportunity_census_v077a"
CACHE = OUT / "cache"

OUT_OPPS = OUT / "pair17_candidate_plate_opportunities_v077a.csv"
OUT_SOL = OUT / "pair17_candidate_solution_coverage_v077a.csv"
OUT_PLATES = OUT / "pair17_independent_plate_inventory_v077a.csv"
OUT_QUERIES = OUT / "pair17_tap_query_manifest_v077a.csv"
OUT_JSON = OUT / "pair17_applause_independent_plate_opportunity_census_v077a.json"
...
def tap_query(label, query):
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        try:
            ...
            # APPLAUSE/DaCHS query-translation 4xx errors are deterministic.
            # Retry rate limiting only; do not repeat an invalid ADQL query five times.
            if int(e.code) != 429:
                raise last from e
...
def or_filter(column, values):
    ...
    if not vals:
        fail(f"REFUSING: empty filter for {column}")
    return "(" + " OR ".join(f"{column}={x}" for x in vals) + ")"


def exposure_query(plate_ids):
    # Operational v077a repair: use the exact simple-TAP form already proven
    # by the installed v058 APPLAUSE transport: no aliases, no cross-table join,
    # deterministic OR filter. Archive names/institutes are joined locally below.
...
def main():
    print("=" * 132)
    print("PAIR 17 — APPLAUSE INDEPENDENT PHYSICAL-PLATE OPPORTUNITY CENSUS v077a")
    print("=" * 132)
    print("Metadata-only census. Operational repair of v077 exposure TAP query.")
    ...
    fov_rows = tap_query("global_solution_max_fov_v077a", max_fov_query())
    ...
    sol_rows = tap_query(
        "candidate_region_solution_scan_superset_v077a",
    ...
    # v077a changes transport syntax/chunking only: the physical plate universe and
    # eligibility semantics are unchanged.
    archive_rows = tap_query("archive_inventory_v077a", archive_query())
    archive_by_id = {}

    for r in archive_rows:
        aid = fint(r.get("archive_id"))
        if aid is not None:
            archive_by_id[aid] = {
    ...
    # The APPLAUSE simple-TAP parser is sensitive to complex/long ADQL. Fifty
    # physical-plate OR terms keeps each query bounded while retrieving the exact
    # same plate-id set frozen by the v077 solution-cap result.
    chunk_size = 50
    for start in range(0, len(plate_ids), chunk_size):
        chunk = plate_ids[start:start + chunk_size]
        label = f"plate_exposures_{start//chunk_size:04d}_v077a"
        rows = tap_query(label, exposure_query(chunk))

        for r in rows:
            aid = fint(r.get("archive_id"))
            meta = archive_by_id.get(aid, {})
            r["archive_name"] = meta.get("archive_name", "")
            r["institute"] = meta.get("institute", "")
    ...
    # v077 attempted to retain ~603 x N_solution rows in RAM. v077a streams this
    # prospectively required audit table to disk while keeping only the best
    # candidate x physical-plate opportunity in memory. Scientific classification
    # logic is byte-for-byte equivalent below.
    ...
    sol_fields = [
        "raw_match_row", "population", "locus_ra_deg", "locus_dec_deg",
        "plate_id", "solution_id", "scan_id", "archive_id", "solution_num",
        "solution_ra_icrs", "solution_dec_icrs", "fov1", "fov2",
        "filename_scan", "naxis1", "naxis2", "file_size", "fits_checksum",
        "footprint_provenance", "coverage_class", "edge_distance_arcsec",
        "independent_physical_plate", "scan_metadata_present",
        "eligible_independent_comparison",
    ]

    OUT.mkdir(parents=True, exist_ok=True)
    sol_tmp = OUT_SOL.with_suffix(OUT_SOL.suffix + ".tmp")
    solution_coverage_row_count = 0

    with sol_tmp.open("w", encoding="utf-8", newline="") as sol_fh:
        sol_writer = csv.DictWriter(sol_fh, fieldnames=sol_fields)
        sol_writer.writeheader()
        ...
        for ci, target in enumerate(loci, 1):
            ...
            if ci % 50 == 0 or ci == len(loci):
                print(
                    f"Coverage evaluated {ci}/{len(loci)} candidates; "
                    f"solution-audit rows streamed={solution_coverage_row_count:,}",
                    flush=True,
                )

    sol_tmp.replace(OUT_SOL)
    ...
    # Write compact outputs. The all-solution audit CSV was streamed atomically above.
    ...
    report = {
        ...
        "operational_repair_of": "v077",
        "operational_repair": {
            "science_contract_changed": False,
            "eligibility_semantics_changed": False,
            "repair_reason":
                "APPLAUSE TAP HTTP 400 translating aliased cross-table exposure query",
            "exposure_transport":
                "unaliased applause_dr4.exposure simple-TAP query with 50 plate-id OR terms; archive metadata joined locally",
            "solution_coverage_output":
                "streamed to disk to avoid retaining millions of audit rows in RAM",
        },
        "population": {
            "all": EXPECTED_TOTAL,
        ...
        "solution_query": {
            ...
            "solution_candidate_audit_rows": solution_coverage_row_count,
    ...
    print()
    print("=" * 132)
    print("v077a OPPORTUNITY CENSUS COMPLETE")
    print("=" * 132)
    ...
    print("STAGE STATUS: COMPLETE (v077a operational repair)")
