...
# Existing frozen peaks only need to be kept far enough away that they
# cannot satisfy the 3 px recovery association or compete within the
# 7 px local-maximum neighbourhood. Eight pixels is therefore a
# conservative, geometry-derived exclusion rather than an outcome-tuned one.
...
def main():
    ...
    print(
        ...
        "(amended from infeasible 40px before any recovery outcome)"
    )
    print()
    ...
    # ---------------------------------------------------------------
    # Position preflight for ALL eight endpoint backgrounds.
    # No synthetic source is injected until every endpoint has exactly
    # the frozen 24 valid positions.
    # ---------------------------------------------------------------
    endpoint_preflight = {}

    print("Preflighting deterministic injection positions on all 8 endpoints ...")
        ...
        for archive in ("POSS", "DASCH"):
            ...
            if len(positions) != N_INJECTION_POSITIONS:
                raise RuntimeError(
                    ...
                    f"{len(positions)} positions; expected {N_INJECTION_POSITIONS}"
                )
            ...
            print(
                f"  [preflight rank {rank:02d} {archive:5s}] "
                f"tile={tile_id} baseline_peaks={len(baseline['x'])} "
                f"positions={len(positions)} PASS",
                flush=True,
            )

    if len(endpoint_preflight) != 8:
        raise RuntimeError(
            f"REFUSING: endpoint preflight count {len(endpoint_preflight)} != 8"
        )

    print("All 8 endpoint position preflights: PASS")
    print("Beginning fixed injection/recovery grid ...")
    print()
    ...
    out_report = {
        ...
        "fixed_injection_design": {
            ...
            "control_design_amendment": {
                "prior_exclusion_radius_px": 40.0,
                "amended_exclusion_radius_px": BASELINE_EXCLUSION_RADIUS_PX,
                "reason": (
                    "The initial 40 px baseline-peak exclusion made the "
                    "predeclared 24-position control geometrically infeasible "
                    "on the first dense POSS tile. The run failed before any "
                    "injection/recovery outcome. The replacement 8 px radius "
                    "is derived from the frozen 3 px recovery radius and 7 px "
                    "local-maximum window, not from recovery outcomes."
                ),
                "prior_run_produced_injection_outcomes": False,
            },
