...
SOLUTION_RAW_VOTABLE = CACHE / "applause_solution_unresolved_v058.xml"
SOLUTION_QUERY_FILE = CACHE / "applause_solution_unresolved_v058.adql"
SOLUTION_QUERY_META = CACHE / "applause_solution_unresolved_v058.meta.json"

SCAN_RAW_VOTABLE = CACHE / "applause_scan_unresolved_v058.xml"
SCAN_QUERY_FILE = CACHE / "applause_scan_unresolved_v058.adql"
SCAN_QUERY_META = CACHE / "applause_scan_unresolved_v058.meta.json"
...
def or_filter(column, values):
    ...
    if not vals:
        raise RuntimeError(f"REFUSING: empty filter for {column}")
    return "(" + " OR ".join(f"{column}={x}" for x in vals) + ")"


def make_exposure_query(exposure_ids):
    where = or_filter("exposure_id", exposure_ids)
    return f"""SELECT
  exposure_id,
  plate_id,
  archive_id,
...
def make_solution_query(plate_ids):
    ...
    *,
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        try:
            ...
            # Query/ADQL errors are deterministic. Preserve the server message
            # instead of retrying the same bad request five times.
            if int(exc.code) != 429:
                break
            ...
            if attempt < MAX_ATTEMPTS:
                time.sleep(min(20.0, 2.0 ** attempt))
    raise RuntimeError(
        f"APPLAUSE TAP query failed after attempt {attempt}: "
...
def main():
    print("=" * 132)
    print("WIDE CENSUS — APPLAUSE GEOMETRY-HOLD OFFICIAL METADATA INVENTORY v058 (SIMPLE-TAP QUERY FIX 3)")
    print("=" * 132)
    ...
    print("Querying official applause_dr4.solution ...", flush=True)

    solution_q = make_solution_query(plate_ids)
    solution_rows, solution_qmeta = tap_query(
        solution_q,
        query_file=SOLUTION_QUERY_FILE,
        raw_votable=SOLUTION_RAW_VOTABLE,
        query_meta=SOLUTION_QUERY_META,
    )
    print(f"Official solution rows returned:           {len(solution_rows)}")

    print("Querying official applause_dr4.scan ...", flush=True)
    scan_q = make_scan_query(plate_ids)
    scan_rows, scan_qmeta = tap_query(
        scan_q,
        query_file=SCAN_QUERY_FILE,
        raw_votable=SCAN_RAW_VOTABLE,
        query_meta=SCAN_QUERY_META,
    )
    print(f"Official scan rows returned:               {len(scan_rows)}")

    scan_by_id = {}
    for r in scan_rows:
        sid = inum(r.get("scan_id"))
        if sid is None:
            raise RuntimeError("REFUSING: official scan row lacks scan_id")
        if sid in scan_by_id:
            raise RuntimeError(f"REFUSING: duplicate official scan_id {sid}")
        scan_by_id[sid] = r

    query_rows = []
    missing_scan_ids = []
    for s in solution_rows:
        sid = inum(s.get("scan_id"))
        scan = scan_by_id.get(sid)
        if scan is None:
            missing_scan_ids.append(sid)
            continue
        if (
            inum(s.get("plate_id")) != inum(scan.get("plate_id"))
            or inum(s.get("archive_id")) != inum(scan.get("archive_id"))
        ):
            raise RuntimeError(
                f"REFUSING: solution/scan physical identity mismatch for scan_id {sid}"
            )
        query_rows.append({
            **s,
            "filename_scan": scan.get("filename_scan"),
            "naxis1": scan.get("naxis1"),
            "naxis2": scan.get("naxis2"),
            "file_size": scan.get("file_size"),
            "fits_checksum": scan.get("fits_checksum"),
        })

    if missing_scan_ids:
        raise RuntimeError(
            "REFUSING: official solution rows reference scan_ids not returned "
            f"by exact plate-scoped scan query: {sorted(set(missing_scan_ids))}"
        )

    print(f"Exact local solution↔scan joins:            {len(query_rows)}")

    # Preserve exact joined metadata. The join is local and keyed only by the
    # official unique scan_id; no detector/candidate information participates.
    query_fields = [
    ...
    payload = {
        ...
        "tap": {
            "endpoint": TAP_SYNC,
            "query_dialect_policy": (
                "simple single-table SELECTs; exact integer-id filters expressed "
                "as OR clauses; no IN predicate; no server-side JOIN"
            ),
            ...
            "solution_query": {
                "query_sha256": sha_bytes(solution_q.encode("utf-8")),
                "response": solution_qmeta,
                "returned_rows": len(solution_rows),
            },
            "scan_query": {
                "query_sha256": sha_bytes(scan_q.encode("utf-8")),
                "response": scan_qmeta,
                "returned_rows": len(scan_rows),
            },
            "local_solution_scan_join": {
                "key": "scan_id",
                "joined_rows": len(query_rows),
                "missing_scan_ids": [],
                "physical_plate_archive_identity_checked": True,
            },
        },
        "footprint_sources": {
        ...
        "outputs": {
            ...
            "solution_query_adql": str(SOLUTION_QUERY_FILE.relative_to(ROOT)).replace("\\", "/"),
            "solution_raw_votable_cache": str(SOLUTION_RAW_VOTABLE.relative_to(ROOT)).replace("\\", "/"),
            "scan_query_adql": str(SCAN_QUERY_FILE.relative_to(ROOT)).replace("\\", "/"),
            "scan_raw_votable_cache": str(SCAN_RAW_VOTABLE.relative_to(ROOT)).replace("\\", "/"),
        },
