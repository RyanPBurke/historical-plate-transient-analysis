...
STAGE2_REPORT = BASE / "order61_platephot_stage2_report.json"
STAGE2_POLICY = BASE / "order61_platephot_stage2_policy.json"
STAGE2_CUM_MANIFEST = BASE / "order61_platephot_stage2_cumulative_manifest.csv"
STAGE2_PLATES_NEW = BASE / "order61_platephot_stage2_plate_summary_new.csv"

CUM_MANIFEST = BASE / "order61_platephot_stage3_cumulative_manifest.csv"
NEW_MANIFEST = BASE / "order61_platephot_stage3_new_call_manifest.csv"
POLICY = BASE / "order61_platephot_stage3_policy.json"
DETAIL_NEW = BASE / "order61_platephot_stage3_detail_new.csv"
PLATES_NEW = BASE / "order61_platephot_stage3_plate_summary_new.csv"
RANK_CUM = BASE / "order61_platephot_stage3_rank_summary_cumulative.csv"
REPORT = BASE / "order61_platephot_stage3_report.json"

for d in (WORK, CACHE):
    d.mkdir(parents=True, exist_ok=True)

API = "https://api.starglass.cfa.harvard.edu/public/dasch/dr7/platephot"
UA = "historical-transient-pipeline/0.2.8-order61-platephot-stage3"
...
STAGE2_CUM_MANIFEST_SHA = (
    "cd773028e71ff7b4fde41f3695157367b"
    "ec6fcc8704655ed00d8d0fa459b3c5e"
)
STAGE2_NEW_MANIFEST_SHA = (
    "1896d6522e8e52eb39d7ae6dd023362b"
    "f2dd9e15a1e8a8e2b2434cbcee565fde"
)
STAGE2_POLICY_SHA = (
    "52fa2e497efafbda28aea03e51580bbfe"
    "c069e503a07892aff56e2a3d873cbee"
)
...
def main():
    print("=" * 98)
    print("ORDER 61 — DASCH PLATEPHOT RECURRENCE STAGE 3: SAME BLIND PREFIX TO 1024")
    print("=" * 98)
    print(
        "Ranks 11/14/20 only: recurrence absent through cumulative 256. "
    ...
    guards = {
        ...
        "stage2_policy_sha": sha256_file(STAGE2_POLICY) == STAGE2_POLICY_SHA,
        ...
        "stage2_same_spatial_gates": (
            ...
            and float(stage2_policy.get("diagnostic_arcsec")) == DIAGNOSTIC_ARCSEC
        ),
    }
    ...
    for rank in ACTIVE_RANKS:
        ...
        if s2_by_rank[rank]["multi_independent_plate_recurrence_5arcsec"]:
            raise RuntimeError(
                f"REFUSING: rank {rank} already met cumulative Stage-2 "
                "5-arcsec recurrence"
            )
        if int(s2_by_rank[rank]["cumulative_completed_plates"]) != STAGE2_CUMULATIVE_PREFIX:
            raise RuntimeError(
                f"REFUSING: rank {rank} Stage-2 cumulative plate count is not 256"
            )
    ...
    for rank in ACTIVE_RANKS:
        ...
        cumulative.extend(ordered[rank][:STAGE3_CUMULATIVE_PREFIX])
        new_calls.extend(
            ordered[rank][STAGE2_CUMULATIVE_PREFIX:STAGE3_CUMULATIVE_PREFIX]
        )
    ...
    policy_obj = {
        "analysis_kind": "order61_platephot_recurrence_stage3_fixed",
        "active_ranks": ACTIVE_RANKS,
        "stage1_recurrent_rank_audit_only": 22,
        "stage2_cumulative_prefix_per_active_rank": STAGE2_CUMULATIVE_PREFIX,
        "stage3_cumulative_prefix_per_active_rank": STAGE3_CUMULATIVE_PREFIX,
    ...
    print("Stage-2 cumulative prefix reproduction for ranks 11/14/20: PASS")
    ...
    print("Manifests/policy frozen before first Stage-3 API request: PASS")
    print()
    ...
    # Cumulative 64 + Stage2 new 192 + Stage3 new 768 summaries.
    cumulative_plate_rows = [
        r for r in stage1_plates
        if int(r["strict_rank"]) in ACTIVE_RANKS
    ] + stage2_plates_new + plates_new
    ...
    for rank in ACTIVE_RANKS:
        ...
        rank_rows.append({
            "strict_rank": rank,
            "cumulative_selected_plates": STAGE3_CUMULATIVE_PREFIX,
            ...
            "stage3_complete": (
                len(done) == STAGE3_CUMULATIVE_PREFIX and len(fail) == 0
            ),
        })
    ...
    report = {
        "status": status,
        "analysis_kind": "order61_platephot_recurrence_stage3",
        "guards": guards,
        "policy_sha256": policy_sha,
        "cumulative_manifest_sha256": cumulative_sha,
        "new_call_manifest_sha256": new_sha,
        "rank22_stage1_disposition": rank22,
        "active_rank_summaries_cumulative_1024": rank_rows,
        ...
        "next_stage_if_complete": (
            "For ranks still without recurrence at cumulative 1024, retain "
            "the recurrence null as a strong historical control and move to "
            "discovery-plate/manual morphology and physical-interpretation "
            "adjudication before considering exhaustive ~4000-plate expansion. "
            "Do not alter the 3/5-arcsec gates or prior samples."
        ),
    }
    ...
    print()
    print("=" * 98)
    print("ORDER 61 PLATEPHOT RECURRENCE STAGE 3 " + status)
    print("=" * 98)
    print(
