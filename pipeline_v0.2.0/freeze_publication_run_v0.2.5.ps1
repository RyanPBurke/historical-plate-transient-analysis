...
Write-Host "Running v0.2.5 tests..."
& $python -m pytest tests -q
if ($LASTEXITCODE -ne 0) { throw "pytest failed" }

$migration = ".\research\EVIDENCE_INDEX_MIGRATION_v0.2.2_2026-08-20.json"
if (-not (Test-Path $migration)) { throw "v0.2.2 evidence migration record is missing; do not freeze v0.2.5." }
...
$availabilityValidation = ".\research\POSS1_XO197_ARCHIVE_EXCEPTION_VALIDATION_v0.2.5.json"
foreach ($p in @($correction,$availabilityManifest,$availabilityPolicy,$skyProbe,$stsciProbe)) {
    if (-not (Test-Path $p)) { throw "Missing v0.2.5 freeze input: $p" }
}
...
Write-Host "Registering v0.2.5 archive-availability evidence..."
& $exe register-artifact --file $availabilityManifest --kind poss1_archive_availability_exception_manifest --stage publication_freeze_v0.2.5 --snapshot
if ($LASTEXITCODE -ne 0) { throw "availability manifest evidence registration failed" }
& $exe register-artifact --file $availabilityPolicy --kind poss1_archive_availability_policy --stage publication_freeze_v0.2.5 --snapshot
if ($LASTEXITCODE -ne 0) { throw "availability policy evidence registration failed" }
& $exe register-artifact --file $skyProbe --kind poss1_xo197_skyview_coverage_probe --stage publication_freeze_v0.2.5 --snapshot
if ($LASTEXITCODE -ne 0) { throw "SkyView XO197 probe evidence registration failed" }
& $exe register-artifact --file $stsciProbe --kind poss1_xo197_stsci_poss1_blue_probe --stage publication_freeze_v0.2.5 --snapshot
if ($LASTEXITCODE -ne 0) { throw "STScI XO197 probe evidence registration failed" }
& $exe register-artifact --file $availabilityValidation --kind poss1_xo197_archive_exception_validation --stage publication_freeze_v0.2.5 --snapshot
...
& $exe register-artifact --file $correction --kind poss1_identity_gate_correction --stage publication_freeze_v0.2.5 --snapshot
if ($LASTEXITCODE -ne 0) { throw "v0.2.4 correction-note evidence registration failed" }
& $exe register-artifact --file $controls --kind skyview_dss_mirror_equivalence_controls --stage publication_freeze_v0.2.5 --snapshot
if ($LASTEXITCODE -ne 0) { throw "control-summary evidence registration failed" }
& $exe register-artifact --file $jar --kind skyview_control_reader_jar --stage publication_freeze_v0.2.5 --source-url "https://skyview.gsfc.nasa.gov/jar/skyview.jar"
if ($LASTEXITCODE -ne 0) { throw "SkyView JAR evidence registration failed" }
& $exe register-artifact --file $dss1b --kind skyview_dss1b_descriptor --stage publication_freeze_v0.2.5 --source-url "https://skyview.gsfc.nasa.gov/current/jar/surveys/xml/dss1b.xml.gz" --snapshot
if ($LASTEXITCODE -ne 0) { throw "DSS1B descriptor evidence registration failed" }
& $exe register-artifact --file $manifest --kind skyview_survey_manifest --stage publication_freeze_v0.2.5 --source-url "https://skyview.gsfc.nasa.gov/current/jar/surveys/survey.manifest" --snapshot
if ($LASTEXITCODE -ne 0) { throw "SkyView manifest evidence registration failed" }

Write-Host "Verifying evidence store before v0.2.5 snapshot..."
& $exe verify-evidence --root .\evidence
if ($LASTEXITCODE -ne 0) { throw "evidence verification failed before v0.2.5 snapshot" }

$out = ".\research_snapshots\sub5_production_freeze_v0.2.5_2026-08-21"
...
& $exe --db .\state\publication_regression_v025.sqlite starglass `
  --manifest .\examples\regression_cases.csv `
  --plate bi05607 `
  --cache-dir .\cache\verified_starglass `
  --export .\results\publication_regression_v025.csv
if ($LASTEXITCODE -ne 0) { throw "regression execution failed" }
& $exe verify-regressions --results .\results\publication_regression_v025.csv
if ($LASTEXITCODE -ne 0) { throw "regression comparison failed" }

Write-Host ""
Write-Host "PUBLICATION FREEZE v0.2.5 PASSED."
...
Write-Host "Next: run .\repair_poss1_identity_state_v0.2.5.ps1, then .\run_poss1_identity_preflight.ps1; no transient detection yet."
