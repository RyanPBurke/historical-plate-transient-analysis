...
V094Y_PROOF_REL = Path('research/proof_archives/v094y_rc4_proof_archive.zip')
...
CONTRACT = HERE / "v095a_stageA_contract_rc9.json"
SQL_FILE = HERE / "v095a_stageA_queries_rc9.sql"
...
RELEASE_REL = Path("pipeline_v0.2.0/research/prospective_freezes/v095a_stageA_aggregate_inventory_rc9")
RC4_INCIDENT_REL = Path("work/v095a_stageA_aggregate_inventory_rc4/jobs/Q0_SELECTED_SOLUTION_BINDING/batch_0001/meta.json")
...
def verify_release_manifest():
    ...
    for line in RELEASE_MANIFEST.read_text(encoding="utf-8-sig").splitlines():
        ...
        entries[name] = digest
    required = {
        "v095a_source_matcher_design_rc9.md", "v095a_stageA_contract_rc9.json",
        "v095a_stageA_queries_rc9.sql", "v095a_stageA_bins_rc9.json",
        "run_v095a_stageA_inventory_rc9.py", "create_v095a_stageA_rc9_freeze_evidence.ps1",
        "create_v095a_rc4_unknown_submission_disposition.ps1",
        "run_v095a_stageA_rc9_selftest.ps1", "run_v095a_stageA_rc9_preflight.ps1",
        "run_v095a_stageA_rc9_production.ps1", ".gitattributes",
        "v095a_runtime_rc9.py", "v095a_jobs_rc9.py", "test_v095a_rc9.py",
        "test_sql_rc9.mjs", "package.json", "package-lock.json",
    }
...
def self_test():
    ...
    import unittest
    import test_v095a_rc9
    suite = unittest.defaultTestLoader.loadTestsFromModule(test_v095a_rc9)
    result = unittest.TextTestRunner(verbosity=2).run(suite)
    if not result.wasSuccessful():
        raise SystemExit("SELFTEST_HOLD: regression failure")
    print("v095a Stage A RC9 SELF-TEST PASS")
    print("Python lifecycle/parser/lock regressions passed; PostgreSQL SQL suite is test_sql_rc9.mjs")
...
def preflight(project: Path, repo: Path, write_plan=True, owner=None):
    ...
    if contract.get("contract_id") != "v095a_stageA_aggregate_inventory_rc9":
        raise SystemExit("RELEASE_HOLD: contract id")
    proof = project / V094Y_PROOF_REL
    ...
    if write_plan:
        work = project / "work/v095a_stageA_aggregate_inventory_rc9"
        plan_dir = work / "plan"
...
def verify_prior_rc4_incident(project: Path, evidence: Path) -> str:
    """Verify explicit user acknowledgement of the preserved RC4 unknown POST.

    This does not reclassify the RC4 submission as rejected or completed. It
    records that one read-only aggregate Q0 POST may have been accepted and that
    the user explicitly authorizes a superseding Stage A attempt with that
    uncertainty preserved in provenance. No TAP request is made here.
    """
    evidence = Path(evidence).resolve()
    if not evidence.is_file():
        raise SystemExit("PRIOR_INCIDENT_HOLD: acknowledgement evidence missing")
    ev = read_json(evidence)
    required = {
        "schema_version": 1,
        ...
        "source_checkpoint_relative": RC4_INCIDENT_REL.as_posix(),
    }
    for key, value in required.items():
        if ev.get(key) != value:
            raise SystemExit(f"PRIOR_INCIDENT_HOLD: invalid {key}")
    source = (project / RC4_INCIDENT_REL).resolve()
    try:
        source.relative_to(project.resolve())
    except ValueError:
        raise SystemExit("PRIOR_INCIDENT_HOLD: checkpoint path escaped project")
    if not source.is_file():
        raise SystemExit("PRIOR_INCIDENT_HOLD: preserved RC4 checkpoint missing")
    if ev.get("source_checkpoint_sha256") != sha(source):
        raise SystemExit("PRIOR_INCIDENT_HOLD: RC4 checkpoint hash changed")
    meta = read_json(source)
    if meta.get("family") != "Q0_SELECTED_SOLUTION_BINDING" or meta.get("batch") != 1:
        raise SystemExit("PRIOR_INCIDENT_HOLD: RC4 batch identity mismatch")
    if meta.get("status") != "FAILED_HOLD" or not str(meta.get("failure", "")).startswith("SUBMISSION_UNCERTAIN_HOLD"):
        raise SystemExit("PRIOR_INCIDENT_HOLD: RC4 failure state mismatch")
    if meta.get("transport_request_attempts") != 1:
        raise SystemExit("PRIOR_INCIDENT_HOLD: RC4 request count mismatch")
    attempts = meta.get("attempts") or []
    if len(attempts) != 1 or attempts[0].get("attempt") != 1 or attempts[0].get("state") != "SUBMITTING":
        raise SystemExit("PRIOR_INCIDENT_HOLD: RC4 attempt state mismatch")
    if attempts[0].get("job_url"):
        raise SystemExit("PRIOR_INCIDENT_HOLD: RC4 unexpectedly has a job URL; reconcile instead")
    if ev.get("recorded_created_utc") != meta.get("created_utc") or ev.get("recorded_updated_utc") != meta.get("updated_utc"):
        raise SystemExit("PRIOR_INCIDENT_HOLD: RC4 checkpoint timestamps mismatch")
    return sha(evidence)
...
def run_batch(session, family, batch_no, query_bytes, expected_fields, validator, expected_arg,
    ...
    from v095a_jobs_rc9 import run_batch as execute_batch
...
def copy_validated_jobs(work, staging, batch_counts, publication_specs, context):
    from v095a_jobs_rc9 import validate_checkpoint
    cumulative = Counter()
    for family, count in batch_counts.items():
        for number in range(1, count+1):
            source = work / 'jobs' / family / f'batch_{number:04d}'
            meta = read_json(source / 'meta.json')
            spec = publication_specs.get((family, number))
            if spec is None:
                raise SystemExit('PUBLICATION_HOLD: missing expected publication checkpoint specification')
            if meta.get('family') != family or meta.get('batch') != number:
                raise SystemExit('PUBLICATION_HOLD: checkpoint batch identity mismatch')
            try:
                rows = validate_checkpoint(source, meta, spec['query'], spec['fields'],
                                           parse_csv_bytes, spec['validator'], spec['expected'], context)
                if rows is None:
                    raise ValueError('checkpoint is not completed/validated')
            except (ValueError, KeyError, TypeError, OSError, json.JSONDecodeError, SystemExit) as exc:
                raise SystemExit('PUBLICATION_HOLD: invalid complete checkpoint lifecycle evidence') from exc
...
def run_production(project: Path, repo: Path, freeze_commit: str, freeze_evidence: Path, prior_incident_evidence: Path, token_env: str):
    with ProjectLock(project) as owner:
        return _run_production_locked(project, repo, freeze_commit, freeze_evidence, prior_incident_evidence, token_env, owner)


def _run_production_locked(project, repo, freeze_commit, freeze_evidence, prior_incident_evidence, token_env, owner):
    owner.require(project)
    prior_incident_sha = verify_prior_rc4_incident(project, prior_incident_evidence)
    if (project / "results/applause_dr4_v095a_stageA_aggregate_inventory_rc9").exists():
    ...
    context = {
        ...
        'prior_rc4_incident_evidence_sha256': prior_incident_sha,
    }
    ...
    for bn, batch in enumerate(chunks(q0_wanted, Q0_BATCH), 1):
        ...
        publication_specs[("Q0_SELECTED_SOLUTION_BINDING", bn)] = {"query": q, "fields": Q0_FIELDS, "validator": validate_q0, "expected": expected}
    ...
    for bn, batch in enumerate(chunks(science, Q1_BATCH), 1):
        ...
        publication_specs[("Q1_SELECTED_SCIENCE_KEY_AGGREGATES", bn)] = {"query": q, "fields": Q1_FIELDS, "validator": validate_q1, "expected": expected}
    ...
    for bn, batch in enumerate(chunks(plate_rows, Q2_BATCH), 1):
        ...
        publication_specs[("Q2_PHYSICAL_PLATE_PROCESSING_MULTIPLICITY", bn)] = {"query": q, "fields": Q2_FIELDS, "validator": validate_q2, "expected": expected}
    ...
    staging = Path(tempfile.mkdtemp(prefix='v095a_rc9_staging_', dir=results))
    final = project / "results/applause_dr4_v095a_stageA_aggregate_inventory_rc9"
    ...
    cumulative = copy_validated_jobs(work, staging, batch_counts, publication_specs, context)
    atomic_json(staging / 'execution_context.json', context)

    globalsums = {f: sum(r[f] for r in q1) for f in Q1_FIELDS[5:]}
    summary = {
        "analysis_kind": "v095a_stageA_aggregate_inventory_rc9",
        "status": "COMPLETE",
        ...
        "v094y_proof_archive_sha256": V094Y_PROOF_SHA,
        "prior_rc4_incident_evidence_sha256": prior_incident_sha,
        "prior_rc4_submission_status": "UNRESOLVED_UNKNOWN_SUBMISSION_ACKNOWLEDGED_FOR_SUPERSEDING_READ_ONLY_RETRY",
    ...
    print("v095a Stage A RC9 PRODUCTION PASS")
...
def main():
    ...
    ap.add_argument("--prior-incident-evidence")
    ...
    if a.preflight_only:
        ...
        print("v095a Stage A RC9 PREFLIGHT PASS")
        ...
        print("catalogue/network calls=0")
        return 0
    if not a.freeze_commit or not a.freeze_evidence or not a.prior_incident_evidence:
        ap.error("production requires --freeze-commit, --freeze-evidence and --prior-incident-evidence")
    return run_production(project, repo, a.freeze_commit, Path(a.freeze_evidence).resolve(),
                          Path(a.prior_incident_evidence).resolve(), a.token_env)
